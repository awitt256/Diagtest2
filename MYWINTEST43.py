...
def show_hardware_test_screen():
    ...
    ctk.CTkLabel(
        top_bar,
        text="Hardware Test Suite — Revision 0.43  Added ADDED LIGHT AND DARK MODE ADDED OPERATORS AND COLORS  ",
    ...
    _timer_label.pack(side="right", padx=18, pady=8)
    ...
    # Theme management
    _current_theme = ["dark"]  # Using list for mutable reference in closures
    ...
    _theme_header = ctk.CTkFrame(sidebar, fg_color="transparent")
    _theme_header.pack(fill="x", padx=10, pady=(10, 4))
    ctk.CTkLabel(_theme_header, text="🎨 Theme:",
                 font=ctk.CTkFont(size=12, weight="bold"),
                 text_color="#8b949e").pack(side="left", padx=(6, 0))
    ...
    _theme_dropdown.pack(side="right", padx=(0, 4))
    ...
    try:
        operator_card.pack_forget()
    except Exception:
        pass
    
    # Header
    op_header_row = ctk.CTkFrame(operator_card, fg_color="transparent")
    op_header_row.pack(fill="x", padx=14, pady=(10,6))
    ...
    operator_status.pack(anchor="w", padx=14, pady=(0,10))
    
    # Operator buttons
    op_buttons_frame = ctk.CTkFrame(operator_card, fg_color="transparent")
    op_buttons_frame.pack(fill="x", padx=14, pady=(0,10))
    ...
    ag_sys_row.pack(fill="x", padx=14, pady=(8, 4))
    ...
    # ══════════════════════════════════════════════════════════════════
    comp_card = card(comp_network_row, "🧩  Components", track_key="comp")
    try:
    ...
    def _make_net_section(parent, title):
        section = ctk.CTkFrame(parent, fg_color="#11161d", corner_radius=8)
        ...
        detail_rows = [
            ("Name", "white"),
            ("Description", "#c9d1d9"),
            ("Adapter status", "white"),
            ("Active", "white"),
            ("Connected to network", "white"),
            ("Network name", "#c9d1d9"),
            ("Network category", "#c9d1d9"),
        ]
        for label_text, color in detail_rows:
            lbl = ctk.CTkLabel(
                section,
                text=f"{label_text}: --",
                font=ctk.CTkFont(size=11),
    ...
    def _set_net_section(section_widgets, adapter):
        ...
        for idx, text in enumerate(label_values):
            try:
                color = connected_color if idx == 4 else labels[idx].cget("text_color")
    ...
    mic_embed_host = tk.Frame(mic_card, bg="#101723", height=300)
    ...
    try:
        ...
        if 'MicTest2' in globals() and MicTest2 is not None:
            try:
                proxy = _FrameWindowProxy(mic_embed_host, bg="#07111f")
    ...
    usb_info_frame = ctk.CTkFrame(usb_card, fg_color="#0d1117", corner_radius=8)
    usb_info_frame.pack(fill="x", padx=14, pady=(0, 10))

    usb_summary_lbl = ctk.CTkLabel(
        usb_info_frame, 
    ...
    usb_connections_lbl = ctk.CTkLabel(
        usb_info_frame, 
    ...
    nfc_status_lbl = ctk.CTkLabel(nfc_card, text="Checking for NFC readers...", font=ctk.CTkFont(size=12), text_color="#9fb3c8")
    nfc_status_lbl.pack(anchor="w", padx=14, pady=(0, 6))

    nfc_info = ctk.CTkFrame(nfc_card, fg_color="#0d1117", corner_radius=8)
    nfc_info.pack(fill="x", padx=14, pady=(0, 10))
    nfc_svc = ctk.CTkLabel(nfc_info, text="Service: --", font=ctk.CTkFont(size=11), text_color="#9fb3c8")
    nfc_svc.pack(anchor="w", padx=12, pady=(10, 4))
    nfc_dev = ctk.CTkLabel(nfc_info, text="NFC Devices: -- | Contactless: --", font=ctk.CTkFont(size=11), text_color="#9fb3c8")
    nfc_dev.pack(anchor="w", padx=12, pady=(4, 10))
    nfc_txt = tk.Text(nfc_info, bg="#0d1117", fg="#c9d1d9", font=("Consolas", 10), height=6, wrap="word", borderwidth=0, highlightthickness=0)
    ...
    fp_info = ctk.CTkFrame(fingerprint_card, fg_color="#0d1117", corner_radius=8)
    fp_info.pack(fill="x", padx=14, pady=(0, 10))
    fp_svc = ctk.CTkLabel(fp_info, text="Service: --", font=ctk.CTkFont(size=11), text_color="#9fb3c8")
    fp_svc.pack(anchor="w", padx=12, pady=(10, 4))
    fp_dev = ctk.CTkLabel(fp_info, text="Biometric: -- | Fingerprint: --", font=ctk.CTkFont(size=11), text_color="#9fb3c8")
    fp_dev.pack(anchor="w", padx=12, pady=(4, 10))
    fp_txt = tk.Text(fp_info, bg="#0d1117", fg="#c9d1d9", font=("Consolas", 10), height=6, wrap="word", borderwidth=0, highlightthickness=0)
    ...
    kb_canvas = tk.Canvas(kb_scroll_wrap, bg="#101723", highlightthickness=0, height=300)
    kb_canvas.pack(side="top", fill="both", expand=True)

    kb_embed_host = tk.Frame(kb_canvas, bg="#101723")
