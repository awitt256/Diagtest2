...
def return_to_main_menu(event=None):
    """Return to the combined hardware test screen."""
    global active_screen
    try:
        app.unbind("<Return>")
    except Exception:
...
def show_hardware_test_screen():
    ...
    ctk.CTkLabel(top_bar, text="Hardware Test Suite",
                 font=ctk.CTkFont(size=18, weight="bold"),
                 text_color="#f5d76e").pack(side="left", padx=18, pady=8)
    ctk.CTkButton(top_bar, text="Back to Main Menu", width=170, height=30,
                  fg_color="#444444", hover_color="#555555",
                  command=lambda: _go_back()).pack(side="right", padx=12, pady=8)
