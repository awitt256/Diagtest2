...
def show_hardware_test_screen():
    ...
    def scroll_to_widget(widget):
        try:
            # Force update to ensure winfo coordinates are current
            app.update_idletasks()
            
            # Find the canvas and the scrollable content frame
            # In CTkScrollableFrame, the canvas is usually self._canvas
            canvas = getattr(body, "_canvas", None) or getattr(body, "_parent_canvas", None)
            inner_frame = getattr(body, "_scrollable_label", None)
            
            if not canvas or not inner_frame:
                return
            
            # Calculate target Y relative to the scrollable content frame (inner_frame)
            target_y = 0
            curr = widget
            # Traverse up the widget tree to the inner_frame to sum up offsets
            while curr and curr != inner_frame:
                try:
                    target_y += curr.winfo_y()
                except Exception:
                    pass
                curr = curr.master
            
            # Get the total scrollable height (requested height of the content)
            total_h = inner_frame.winfo_reqheight()
    ...
    def scroll_to_widget(widget):
        try:
            # Search for the internal canvas and content frame
            canvas = getattr(body, "_parent_canvas", None) or getattr(body, "_canvas", None)
            content_frame = getattr(body, "_scrollable_label", None)
            
            # Fallback: search children if internals aren't exposed as expected
            if not canvas:
                for child in body.winfo_children():
                    if "canvas" in str(child).lower():
                        canvas = child
                        break
            
            if not canvas or not content_frame:
                return

            # Ensure coordinates are up to date
            app.update_idletasks()
            
            # Calculate position using screen-absolute root coordinates
            # This is the most reliable way to find the relative offset
            try:
                y_offset = widget.winfo_rooty() - content_frame.winfo_rooty()
                total_h = content_frame.winfo_height()
                
                if total_h > 0:
                    fraction = y_offset / total_h
                    canvas.yview_moveto(max(0, min(1, fraction - 0.01)))
            except Exception:
                # Fallback to local winfo_y traversal if rooty fails
                target_y = 0
                curr = widget
                while curr and curr != content_frame:
                    target_y += curr.winfo_y()
                    curr = curr.master
                total_h = content_frame.winfo_height() or 1
    ...
    ctk.CTkLabel(sidebar, text="Test Summary", font=ctk.CTkFont(size=16, weight="bold"),
                 text_color="#58a6ff").pack(pady=(20, 10))
    ...
    def _show_captured_frame(frame):
        ...
        try:
            ...
            panel = tk.Frame(cam_preview_row, bg="#000000", width=180, height=135)
...
"""
    ...
    # ══════════════════════════════════════════════════════════════════
    add_sidebar_item("sys", "System Info", sys_card)
    add_sidebar_item("bat", "Battery", bat_card)
    add_sidebar_item("spk", "Speaker", spk_card)
    add_sidebar_item("mic", "Microphone", mic_card)
    add_sidebar_item("br",  "Brightness", brightness_card)
    add_sidebar_item("tp",  "Touchpad", tp_card)
    ...
    add_sidebar_item("kb",  "Keyboard", kb_card)
