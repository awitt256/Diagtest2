...
def show_hardware_test_screen():
        ...
        def _update():
            ...
            try:
                ...
                for ln in str(report).splitlines():
                    _append_sys_line(ln)
                # Auto-mark System Info card as PASS when loading completes
    ...
    tp_status = ctk.CTkLabel(tp_card, text="Touchpad test",
                              font=ctk.CTkFont(size=12), text_color="#9fb3c8")
    ...
    # Auto-start embedding so the touchpad UI appears without pressing Start
    try:
    ...
    def _start_camera_preview():
        ...
        def update():
            if not cam_preview_row.winfo_exists():
                # host destroyed -> release
                try:
                    for c in _cam_caps_local:
                        c.release()
                except Exception:
                    pass
                return
    ...
    # start preview in background so UI remains responsive
    try:
    ...
    try:
        ...
        try:
            ...
            app.after(0, lambda: messagebox.showerror("Keyboard Embed", f"Could not embed keyboard GUI:\n{exc}"))
