...
def show_hardware_test_screen():
    ...
    try:
        ...
        form_frame.pack(fill="x", padx=14, pady=(4, 8))

        # ── Check Label sub-section ──────────────────────────────────
        check_label_frame = _register_ctk_frame(ctk.CTkFrame(form_frame, fg_color=theme_value("embed_bg"), corner_radius=8))
        check_label_frame.pack(fill="x", pady=(0, 8))
        ...
        _serial_entry_bg = theme_value("embed_bg")
        ...
        def _serial_on_click(event=None):
            try:
                ...
                if _form_serial_placeholder_active[0]:
                    ...
                    entry.configure(state="normal")
                    # Move cursor to end so user types right after base
                    entry.icursor("end")
        ...
        def _serial_on_key(event=None):
            try:
                ...
                # Allow only up to 2 extra characters
                if len(suffix) > 2:
                    entry.delete(len(base) + 2, "end")
                # Prevent deleting the base
        ...
        def _serial_on_backspace(event=None):
            try:
                ...
                cur_pos = entry.index("insert")
                if cur_pos <= len(base):
                    return "break"  # prevent deleting base
            except Exception:
                pass
