...
class MicrophoneTesterApp:
    ...
    CHANNELS = 1
    BLOCKSIZE = 1024
    SOUND_THRESHOLD = 0.003

    def __init__(self, root):
        self._embedded = isinstance(root, tk.Frame)
        self.root = root
    ...
    def _build_ui(self):
        ...
        if self._embedded:
            wave_shell.pack(fill="both", expand=True, padx=8, pady=(8, 6))
        else:
            wave_shell.pack(fill="x", padx=22, pady=(22, 18))

        canvas_height = 144 if self._embedded else 300
        ...
        if self._embedded:
            self.wave_canvas.pack(fill="both", expand=True, padx=8, pady=8)
        else:
            self.wave_canvas.pack(fill="x", padx=14, pady=14)

        meter_row = tk.Frame(panel, bg="#0c1728")
        if self._embedded:
            meter_row.pack(fill="x", padx=10, pady=(0, 6))
        else:
        ...
        tk.Label(
            meter_row,
            text="INPUT LEVEL",
            font=("Segoe UI", 9 if self._embedded else 10, "bold"),
            bg="#0c1728",
            fg="#8ba5c2",
        ).pack(side="left", padx=(0, 10 if self._embedded else 18))
        ...
        self.meter_canvas = tk.Canvas(
            meter_wrap,
            width=720,
            height=12 if self._embedded else 18,
            bg="#111c30",
            bd=0,
            highlightthickness=0,
        )
        ...
        self.start_button = tk.Button(
            controls,
            text="Start" if self._embedded else "Start 5 Second Test",
            command=self.start_test,
            bg="#5ab18e",
            fg="#08111f",
            activebackground="#75c7a5",
            activeforeground="#08111f",
            font=("Segoe UI", 10 if self._embedded else 13, "bold"),
            padx=12 if self._embedded else 20,
            pady=8 if self._embedded else 14,
            bd=0,
            relief="flat",
            cursor="hand2",
        )
    ...
    def _set_status(self, message, is_error=False):
        ...
        if not hasattr(self, "status_frame") or not hasattr(self, "status_label"):
            return
        if is_error:
    ...
    def start_test(self):
        ...
        self.record_deadline = self.record_start_time + self.RECORD_SECONDS
        ...
        if self.stop_playback_button is not None:
    ...
    def _schedule_countdown(self):
        ...
        now = time.time()
        if self.record_deadline is not None and now >= self.record_deadline:
            self.stop_test()
            return
    ...
    def stop_test(self):
        ...
        self._set_status("Recording complete. Playing the recorded clip automatically.")
        self.play_recording()
    ...
    def _schedule_wave_update(self):
        if self.is_recording and self.record_deadline is not None and time.time() >= self.record_deadline:
            self.stop_test()
    ...
    def _drain_audio_queue(self):
        ...
        while True:
            ...
            if active_level > 0:
                self.wave_activity = min(1.0, active_level * 48.0)
                if peak > 0:
                    normalized = payload / max(peak, 0.01)
                    visible_strength = min(1.0, 0.58 + peak * 10.0)
                    self.display_audio = normalized * visible_strength
            else:
                self.display_audio *= 0.82

        if not processed_audio and not self.is_recording:
            self.current_level *= 0.88
            self.wave_peak *= 0.86
            self.display_audio *= 0.82
        self.wave_activity *= 0.9
    ...
    def _draw_live_wave(self):
        ...
        center_y = height / 2
        active = max(self.wave_activity, min(1.0, self.current_level * 1.35), min(1.0, self.wave_peak * 8.0))
        ...
        if active <= 0.015:
            ...
            if len(samples) < 2:
                ...
                amplitude = 28 + active * (height * 0.34)
