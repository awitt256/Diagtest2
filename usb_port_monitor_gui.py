...
def detect_hdmi():
    try:
        ...
        connected = int(monitor_count) > 1
        if connected:
            return True, f"External display detected ({monitor_count} total monitors)", "- Windows reports more than one active monitor"
        return False, "No external display detected", "- Windows reports a single active monitor"
