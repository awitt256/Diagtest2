...
# ------------------------------------------------------------------
# Activation module (ported from HPLENDELLDEV5)
# ------------------------------------------------------------------
def get_windows_activation_status():
    """Return (is_activated: bool, message: str) using PowerShell query."""
    try:
        cmd = [
            'powershell',
            '-NoProfile',
        ...
        if result.returncode == 0:
            license_status = (result.stdout or "").strip()
            if license_status == "1":
        ...
        try:
            # Fallback: try a simpler command
            result = subprocess.run(['powershell', '-NoProfile', '-Command', 'slmgr /xpr'], capture_output=True, text=True, timeout=12)
            out = (result.stdout or "") + (result.stderr or "")
            if 'This machine is permanently activated' in out or 'The machine is permanently activated' in out:
                return True, 'Windows is Activated.'
            return False, out.strip() or 'Unable to determine activation status.'
        except Exception:
...
def show_activation_screen():
    win = ctk.CTkToplevel(app)
    win.title("Windows Activation Status")
    win.geometry("640x360")

    lbl = ctk.CTkLabel(win, text="Checking Windows activation...", font=ctk.CTkFont(size=16, weight="bold"))
    lbl.pack(pady=(12, 8))

    textbox = ctk.CTkTextbox(win, wrap="word")
    textbox.pack(fill="both", expand=True, padx=12, pady=(0, 12))
    textbox.insert("1.0", "Querying activation state...\n")
    textbox.configure(state="disabled")
    ...
    def do_check():
        textbox.configure(state="normal")
        textbox.delete("1.0", "end")
        textbox.insert("1.0", "Checking activation status...\n")
        textbox.configure(state="disabled")

        def worker():
            try:
                is_act, msg = get_windows_activation_status()
            except Exception as exc:
                is_act = False
                msg = f"Error: {exc}"
            ...
            def update_ui():
                ...
                textbox.configure(state="disabled")
                lbl.configure(text="Activation Result")

            app.after(0, update_ui)

        threading.Thread(target=worker, daemon=True).start()

    ctk.CTkButton(btn_row, text="Refresh", width=120, command=do_check).pack(side="left", padx=8)
    ctk.CTkButton(btn_row, text="Close", width=120, fg_color="#444444", hover_color="#555555", command=win.destroy).pack(side="left")
...
def get_main_menu_sections():
    return [
        ("SYSTEM / HARDWARE", [
            ...
            ("Windows Activation", show_activation_screen, ["activation", "windows key"]),
