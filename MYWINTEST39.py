...
def show_hardware_test_screen():
    ...
    ctk.CTkLabel(
        top_bar,
        text="Hardware Test Suite — Revision 0.39: Added Enrollment Check Card",
            ...
            def _apply():
                ...
                try:
                    if total_count > 0 and hasattr(net_card, "set_pass"):
                        net_card.set_pass()
                    elif hasattr(net_card, "set_fail"):
                        net_card.set_fail()
                except Exception:
                    pass
...
"""
    ...
    kb_xscroll = tk.Scrollbar(kb_scroll_wrap, orient="horizontal")
    kb_xscroll.pack(side="bottom", fill="x")
    ...
    kb_xscroll.config(command=kb_canvas.xview)
    kb_canvas.configure(xscrollcommand=kb_xscroll.set)
    ...
    def _kb_horizontal_scroll(event):
        try:
            if getattr(event, "num", None) == 5 or (getattr(event, "delta", 0) < 0):
                kb_canvas.xview_scroll(3, "units")
    ...
    for _w in (kb_canvas, kb_embed_host, kb_scroll_wrap):
        try:
            _w.bind("<Shift-MouseWheel>", _kb_horizontal_scroll, add="+")
            _w.bind("<Shift-Button-4>", _kb_horizontal_scroll, add="+")
            _w.bind("<Shift-Button-5>", _kb_horizontal_scroll, add="+")
        except Exception:
            pass
...
"""
    ...
    sequence_keys = [
        "ag", "sys", "comp", "net", "bat", "spk", "mic", "br", "tp",
