...
import os as _os
_kb2_path = _os.path.join(_os.path.dirname(__file__), '../Documents/Playground/KeyboardTesterGUI2.py')
...
def show_keyboard_tester():
    ...
    KeyboardTesterGUI2.KeyboardTesterApp(active_screen)
    # Bind Ctrl+Alt+X to return to main menu
    def kb_return(event=None):
        return_to_main_menu()
    app.bind_all('<Control-Alt-x>', kb_return)
    # Add a label for user instruction
    info_label = tk.Label(active_screen, text="Press Ctrl+Alt+X to go back to menu", font=("Segoe UI", 12, "bold"), bg="#101723", fg="#fbbf24")
    info_label.pack(pady=(10, 0))


def return_to_main_menu(event=None):
    """Return to main menu (called by F3)"""
    global active_screen
    clear_screen()
    # Remove Ctrl+Alt+X binding to avoid duplicate bindings
    app.unbind_all('<Control-Alt-x>')
    main_frame.pack(pady=10, padx=10, fill="both", expand=True)
    render_main_menu()
