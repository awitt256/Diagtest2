...
def show_hardware_test_screen():
    ...
    ctk.CTkLabel(
        top_bar,
        text="Hardware Test Suite — Revision 0.40: got sequencer and kb sections working",
...
"""
    ...
    def _start_enrollment_check():
        ...
        try:
            ...
            tf.write(EMBEDDED_CHECK_ENROLLMENT_PS1)
            tf.flush()
            tf.close()
            tf_path = tf.name
            cmd = ["powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", tf_path]
            creation = getattr(subprocess, "CREATE_NO_WINDOW", 0)
            proc = subprocess.Popen(
                cmd,
                stdout=subprocess.PIPE,
