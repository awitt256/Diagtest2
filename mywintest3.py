...
def show_hardware_test_screen():
    ...
    ctk.CTkLabel(top_bar, text="Hardware Test Suite — Revision 0.03: Got System info module working",
    ...
    sys_text = ctk.CTkTextbox(sys_card, wrap="word", height=12)
    sys_text.pack(fill="both", expand=True, padx=14, pady=(6, 12))
    ...
    def _load_system_info():
        ...
        if not os.path.exists(script_path):
            ...
            # Run PowerShell in a subprocess, provide newline to stdin, capture output, and enforce timeout
            ...
            try:
                ...
                try:
                    ...
                    report = f"PowerShell timed out after 90s. Partial output:\n\n{out}\n\nErrors:\n{err}"
                else:
                    if proc.returncode != 0:
                        report = f"PowerShell Error (Exit Code {proc.returncode}):\n{err if err else out}"
                    else:
                        report = out if out else (err if err else "No output returned from PowerShell script.")
            except Exception as e:
...
def render_main_menu():
    ...
    ctk.CTkLabel(
        title_frame,
        text="DTT v 0.88 Created By Anthony Witt 2026",
        font=ctk.CTkFont(size=24, weight="bold"),
        text_color="#f8f8f8"
    ).pack(pady=(1, 1))
    ...
    ctk.CTkLabel(
        title_frame,
        text="Revision V0.88 - Added New Battery Test, Mic Test, Sound Test, and Fixed Banner",
        font=ctk.CTkFont(size=12, weight="bold"),
        text_color="#d4af37"
    ).pack(pady=(1, 1))
