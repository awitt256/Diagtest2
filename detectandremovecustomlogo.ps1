...
try {
    ...
    if (-not $allPresent) {
        ...
        # Install HP.Private dependency first with license acceptance
        foreach ($mod in $dependencyModules) {
            if (-not (Get-Module -ListAvailable -Name $mod)) {
                try {
                    Write-Host "  Installing dependency: $mod" -ForegroundColor Yellow
