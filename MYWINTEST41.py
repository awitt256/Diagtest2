...
def show_hardware_test_screen():
    ...
    ctk.CTkLabel(
        top_bar,
        text="Hardware Test Suite — Revision 0.41  Added enrollment test and computrace check refined pdf ",
    ...
    # AUTO ADVANCE: Brightness -> Port Checker
    def _on_br_marked():
        """After Brightness PASS/FAIL, show Port Checker card and start port check."""
        def _show_and_start_port():
            try:
                _highlight_and_show(port_card)
            except Exception:
                pass
            try:
                threading.Thread(target=_start_port_check, daemon=True).start()
            except Exception:
                pass
        ui_call(_show_and_start_port)
    ...
    try:
        if hasattr(brightness_card, 'set_pass') and hasattr(brightness_card, 'pass_btn'):
            ...
            def _br_pass():
                try: _stop_brightness_test()
                except Exception: pass
                _br_orig_pass()
                if _sequence_running[0]:
                    ui_call(lambda: _highlight_and_show(port_card))
                    return
                _on_br_marked()
    ...
    # \u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550
    # PORT CHECKER CARD (full width)
    # \u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550
    port_card = card(body, "🔌  Port Checker", track_key="port")
    try:
        port_card.pack_forget()
    except Exception:
        pass
    try:
        try:
...
"""
    ...
    # AUTO ADVANCE: Port Checker -> Touchscreen
    def _on_port_marked():
        """After Port Checker PASS/FAIL, show Touchscreen card and start touchscreen test."""
        def _show_and_start_ts():
            ...
            try:
                ...
                pass
        ui_call(_show_and_start_ts)

    try:
        if hasattr(port_card, 'set_pass') and hasattr(port_card, 'pass_btn'):
            _port_orig_pass = port_card.set_pass
            _port_orig_fail = port_card.set_fail

            def _port_pass():
                _port_orig_pass()
                if _sequence_running[0]:
                    ui_call(lambda: _highlight_and_show(touchscreen_card))
                    return
                _on_port_marked()
            ...
            port_card.set_pass = _port_pass
            port_card.set_fail = _port_fail
            port_card.pass_btn.configure(command=_port_pass)
            port_card.fail_btn.configure(command=_port_fail)
    except Exception:
        pass
    ...
    try:
        port_card.pack(fill="x", padx=14, pady=8)
    except Exception:
        pass
...
"""
    ...
    sequence_keys = [
        ...
        "port", "ts", "px", "cam", "kb", "act", "drv", "gpu", "enr", "vs"
    ]
