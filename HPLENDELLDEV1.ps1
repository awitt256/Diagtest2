...
function Get-CommonSystemData {
    ...
    $os = Get-CimInstance -ClassName SoftwareLicensingProduct -Filter "Name like 'Windows%'" |
...
function Write-CommonSystemInfo {
    ...
    Write-Output "============================="
    if ($Data.Os -and $Data.Os.LicenseStatus -eq 1) {
...
function Get-HpBiosEntries {
    ...
    $outputFile = Join-Path $PSScriptRoot "bcu-output.txt"
    $stdOutFile = Join-Path $PSScriptRoot "bcu-stdout.txt"
    $stdErrFile = Join-Path $PSScriptRoot "bcu-stderr.txt"
    ...
    try {
        ...
        if (-not (Test-Path -LiteralPath $outputFile)) {
            $stdOut = if (Test-Path -LiteralPath $stdOutFile) { (Get-Content -LiteralPath $stdOutFile -Raw -ErrorAction SilentlyContinue).Trim() } else { "" }
            $stdErr = if (Test-Path -LiteralPath $stdErrFile) { (Get-Content -LiteralPath $stdErrFile -Raw -ErrorAction SilentlyContinue).Trim() } else { "" }
        ...
        if (-not $entries) {
            ...
            return @{
                Success = $false
                Message = if ([string]::IsNullOrWhiteSpace($preview)) {
...
function Get-DellCctkPath {
    $candidates = @(
        (Join-Path $PSScriptRoot "CCTK.exe")
        (Join-Path $PSScriptRoot "Command Configure\X86_64\CCTK.exe")
...
function Invoke-DellSection {
    ...
    $outputPath = Join-Path $PSScriptRoot 'Dell.ini'
