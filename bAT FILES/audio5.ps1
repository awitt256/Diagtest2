<# Ensure NuGet provider and AudioDeviceCmdlets module are installed first #>

# Ensure NuGet package provider is installed (avoid interactive prompt)
if (-not (Get-PackageProvider -Name NuGet -ListAvailable -ErrorAction SilentlyContinue)) {
    Write-Host "`nNuGet package provider not found. Installing..." -ForegroundColor Yellow
    try {
        Install-PackageProvider -Name NuGet -MinimumVersion 2.8.5.201 -Force -Scope CurrentUser -ErrorAction Stop
    } catch {
        Write-Host "`nFailed to install NuGet provider: $($_.Exception.Message)" -ForegroundColor Red
        Write-Host "You may need to run PowerShell as Administrator." -ForegroundColor Magenta
...
if ($target) {
    Set-AudioDevice -Index $target.Index | Out-Null
    Write-Host "Switched audio output to: $($target.Name)" -ForegroundColor Green
} else {
    ...
    $logPath = "$PSScriptRoot\audio_script_error.log"
    $timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
    $errorMsg = "$timestamp - Could not find a preferred playback device."
    Add-Content -Path $logPath -Value $errorMsg

    Write-Host "`nA log entry was written to $logPath" -ForegroundColor DarkYellow
    exit 1
}
} catch {
    $errorMessage = "[$(Get-Date -Format u)] Error: $($_.Exception.Message)"
    Add-Content -Path "$PSScriptRoot\audio-switch-errors.log" -Value $errorMessage
    Write-Host "`nAn error occurred: $($_.Exception.Message)" -ForegroundColor Red
    Write-Host "`nDetails have been logged to audio-switch-errors.log" -ForegroundColor Yellow
    Write-Host "`nPress Enter to exit..."
    Read-Host
    exit 1
}
