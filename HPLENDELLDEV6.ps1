...
function Get-ComputraceReport {
    ...
    foreach ($setting in $hpSettings) {
        ...
        if (
            $settingName -match '(?i)Permanent\s*Disable\s*Absolute\s*Persistence\s*Module\s*Set\s*Once' -and
            $currentValue -match '^(?i)(Yes|No)$'
        ) {
            $biosFindings += New-ComputraceFinding -Category 'BIOS' -Name "HP BIOS: $settingName" -Status 'Info' -Details "CurrentValue: $currentValue (pass - module is not activated/enabled)"
            continue
        }
