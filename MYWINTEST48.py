...
def show_hardware_test_screen():
    ...
    wifi_networks_frame.pack(fill="both", expand=True, padx=4, pady=(8, 4))
    ...
    # Treeview for networks list
    try:
        networks_tree = ttk.Treeview(
            wifi_networks_frame,
        ...
        networks_tree.heading("SSID", text="Network Name (SSID)")
        networks_tree.heading("Signal", text="Signal Strength")
        networks_tree.heading("Security", text="Security Type")
        networks_tree.heading("Frequency", text="Frequency")
        
        networks_tree.column("SSID", width=200)
        networks_tree.column("Signal", width=100)
        networks_tree.column("Security", width=120)
        networks_tree.column("Frequency", width=80)
        
        scrollbar = ttk.Scrollbar(wifi_networks_frame, orient=tk.VERTICAL, command=networks_tree.yview)
        networks_tree.configure(yscrollcommand=scrollbar.set)
        
        networks_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(10, 0), pady=(0, 10))
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y, padx=(0, 10), pady=(0, 10))
        
        # Store reference for updates
        net_sections["wifi_networks_tree"] = networks_tree
    except Exception as e:
        print(f"Failed to create networks treeview: {e}")
        net_sections["wifi_networks_tree"] = None
    ...
    def _scan_wifi_networks():
        """Scan for available WiFi networks and populate the treeview"""
        networks_tree = net_sections.get("wifi_networks_tree")
        if not networks_tree or not widget_exists(networks_tree):
            return
        ...
        try:
            # Use netsh to scan for networks (without bssid mode first for cleaner SSID list)
            result = subprocess.run(
            ...
            for i, line in enumerate(lines):
                ...
                if re.match(r'^SSID\s+\d+\s*:', line_stripped, re.IGNORECASE):
                    ...
                    if match:
                        ...
                        # Skip if it's actually a BSSID line or empty
                        if ssid and not re.match(r'^[0-9a-f:]+$', ssid, re.IGNORECASE):
                    ...
                    if match:
                        ...
                        current_network["frequency"] = "2.4 GHz" if channel <= 14 else "5 GHz"
            ...
            for net in networks:
                ...
                if ssid and ssid not in seen_ssids:
                    # Double-check it's not a MAC address
                    if not re.match(r'^[0-9a-f:]+$', ssid, re.IGNORECASE):
            ...
            for network in unique_networks:
                networks_tree.insert("", tk.END, values=(
