...
function Get-CommonSystemData {
    ...
    $displayInfo = Get-CimInstance -Namespace root\wmi -ClassName WmiMonitorID -ErrorAction SilentlyContinue
...
function Get-InternalPhysicalDisks {
    ...
    return @(
        $Disks | Where-Object {
            ...
            $interfaceType -notmatch '^(?i)(USB|1394)$' -and
            $model -notmatch '(?i)\bUSB\b' -and
            $model -notmatch '(?i)(virtual|vhd|vmware|virtualbox|qemu|xen|hyper-?v|iscsi|ram\s*disk|microsoft\s*virtual)' -and
            $mediaType -notmatch '(?i)(removable|external)' -and
            $pnpDeviceId -notmatch '(?i)(^USBSTOR\\|^USB\\|VMBUS\\|PROD_VIRTUAL|HYPER-V)'
        }
    )
}
...
function Convert-MonitorBytesToString {
    param(
        $Value
    )
    ...
    return ([System.Text.Encoding]::ASCII.GetString([byte[]]$bytes)).Trim()
}
...
function Get-ComputraceReport {
    $findings = @()

    $processTargets = @('rpcnet', 'rpcnetp', 'upgrd', 'atrack', 'abtagent')
    foreach ($name in $processTargets) {
        $processes = Get-Process -Name $name -ErrorAction SilentlyContinue
        foreach ($process in $processes) {
            $findings += New-ComputraceFinding -Category 'Process' -Name $process.ProcessName -Status 'Present' -Details "PID $($process.Id)"
        }
    }

    $servicePatterns = @('rpcnet', 'rpcnetp', 'upgrd', 'Absolute*', 'LoJack*')
    foreach ($pattern in $servicePatterns) {
        $services = Get-Service -Name $pattern -ErrorAction SilentlyContinue
        foreach ($service in $services) {
            $findings += New-ComputraceFinding -Category 'Service' -Name $service.Name -Status $service.Status.ToString() -Details "StartType: $($service.StartType)"
        }
    }

    $drivers = Safe-ComputraceCim -ClassName Win32_SystemDriver | Where-Object {
        $_.Name -match 'rpcnet|absolute|computrace|lojack' -or
        $_.DisplayName -match 'rpcnet|absolute|computrace|lojack' -or
        $_.PathName -match 'rpcnet|absolute|computrace|lojack'
    }
    foreach ($driver in $drivers) {
        $findings += New-ComputraceFinding -Category 'Driver' -Name $driver.Name -Status 'Present' -Details "State: $($driver.State); StartMode: $($driver.StartMode)"
    }
    ...
    foreach ($path in ($paths | Where-Object { Test-Path $_ })) {
        $item = Get-Item -LiteralPath $path
        $findings += New-ComputraceFinding -Category 'File' -Name $item.Name -Status 'Present' -Details $item.FullName
    }
    ...
    if ($bios) {
        ...
        if ($biosText -match 'absolute|computrace') {
            $biosFindings += New-ComputraceFinding -Category 'BIOS' -Name 'Win32_BIOS text match' -Status 'Possible' -Details $biosText.Trim()
        }
    }

    $dellAttrs = Safe-ComputraceCim -Namespace 'root/dcim/sysman/biosattributes' -ClassName EnumerationAttribute |
        Where-Object { $_.AttributeName -match 'absolute|computrace|persistence' }
    foreach ($attr in $dellAttrs) {
        $biosFindings += New-ComputraceFinding -Category 'BIOS' -Name "Dell BIOS: $($attr.AttributeName)" -Status 'Reported' -Details "CurrentValue: $($attr.CurrentValue)"
    }

    $lenovoSettings = Safe-ComputraceCim -Namespace 'root/wmi' -ClassName Lenovo_BiosSetting |
        Where-Object { $_.CurrentSetting -match 'absolute|computrace|persistence' }
    foreach ($setting in $lenovoSettings) {
        $biosFindings += New-ComputraceFinding -Category 'BIOS' -Name 'Lenovo BIOS setting' -Status 'Reported' -Details $setting.CurrentSetting
    }

    $hpSettings = Safe-ComputraceCim -Namespace 'root/HP/InstrumentedBIOS' -ClassName HP_BIOSEnumeration |
        Where-Object { $_.Name -match 'absolute|computrace|persistence' }
    foreach ($setting in $hpSettings) {
        $settingName = [string]$setting.Name
        $currentValue = [string]$setting.CurrentValue

        $hpInactivePattern = '(?i)^(No|Inactive|Disabled|Off|Deactivated|Not\s*Active|Not\s*Enabled|False|Absent|Missing|None)$'
        $hpActivePattern = '(?i)\b(Active|Enabled|On|Present|Installed|Activated)\b'

        if ($settingName -match '(?i)Permanent\s*Disable\s*Absolute\s*Persistence\s*Module\s*Set\s*Once') {
            if ($currentValue -match $hpInactivePattern) {
                $biosFindings += New-ComputraceFinding -Category 'BIOS' -Name "HP BIOS: $settingName" -Status 'Info' -Details "CurrentValue: $currentValue (pass - module is inactive or not enabled)"
                continue
            }

            if ($currentValue -match $hpActivePattern) {
                $biosFindings += New-ComputraceFinding -Category 'BIOS' -Name "HP BIOS: $settingName" -Status 'Reported' -Details "CurrentValue: $currentValue (fail - module appears active)"
                continue
            }

            $biosFindings += New-ComputraceFinding -Category 'BIOS' -Name "HP BIOS: $settingName" -Status 'Info' -Details "CurrentValue: $currentValue (pass - module is not currently active)"
            continue
        }

        $biosFindings += New-ComputraceFinding -Category 'BIOS' -Name "HP BIOS: $settingName" -Status 'Reported' -Details "CurrentValue: $currentValue"
    }

    $findings += $biosFindings

    $positiveFindings = @($findings | Where-Object { $_.Status -in @('Present', 'Running', 'Possible', 'Reported') })
    $summary = if ($positiveFindings) {
        "FAIL - Indicators found for Absolute/Computrace."
    }
    else {
        "PASS - No active OS-level Computrace indicators found."
    }
...
function Write-CommonSystemInfo {
    ...
    Write-Output "============="
    if ($Data.DisplayInfo) {
        foreach ($display in $Data.DisplayInfo) {
            $manufacturer = Convert-MonitorBytesToString -Value $display.ManufacturerName
            $name = Convert-MonitorBytesToString -Value $display.UserFriendlyName
            $serial = Convert-MonitorBytesToString -Value $display.SerialNumberID
            $year = if ($display.YearOfManufacture) { $display.YearOfManufacture } else { "Unknown" }

            Write-Output "Manufacturer: $(if ($manufacturer) { $manufacturer } else { 'Unknown' })"
            Write-Output "Name: $(if ($name) { $name } else { 'Unknown' })"
            Write-Output "Serial: $(if ($serial) { $serial } else { 'Unknown' })"
            Write-Output "Manufacture Year: $year"
            Write-Output ""
        }
    }
    else {
        Write-Output "No display information found."
    }
    ...
    Write-Output "=================="
    Write-Output ""
    Write-Output "Computrace Check"
    Write-Output "=================="
    $computrace = Get-ComputraceReport
    Write-Output $computrace.Summary
    if ($computrace.Findings) {
        foreach ($finding in $computrace.Findings) {
            Write-Output "$($finding.Category): $($finding.Name)"
            if ($finding.Details) {
                Write-Output "Details: $($finding.Details)"
            }
        }
    }
    else {
        Write-Output "No Computrace findings reported."
    }
}
    ...
    $featureDefinitions = @(
        ...
        [pscustomobject]@{
            ...
            DeviceCheck = {
                param($Devices)
                $Devices | Where-Object {
                    ($_.ClassName -eq "Net" -or [string]::IsNullOrWhiteSpace($_.ClassName)) -and
        ...
        [pscustomobject]@{
            ...
            DeviceCheck = {
                param($Devices)
                $Devices | Where-Object {
                    $_.ClassName -eq "Keyboard" -and $_.DeviceDescription -match '(?i)backlight'
                } | Select-Object -First 1
            }
        }
        [pscustomobject]@{
            Label = "Privacy"
            BiosPattern = "Sure\s*View|SureView|HP\s*Sure\s*View|Privacy"
            DeviceCheck = {
                param($Devices)
                $Devices | Where-Object {
...
function Invoke-LenovoSection {
    ...
    $featureDefinitions = @(
        ...
        [pscustomobject]@{
            ...
            DeviceCheck = {
                param($Devices)
                $Devices | Where-Object { $_.ClassName -eq "Keyboard" -and $_.DeviceDescription -match '(?i)backlight' } | Select-Object -First 1
            }
        }
    )
...
function Test-FingerprintPnpDevice {
    param(
        [AllowEmptyString()]
        [string]$Text,
    ...
    $windowsHelloPattern = '(windowshello|helloface|facialrecognition|faceauthentication|faceauth|faceenrollment|iriscamera|irisscanner|infraredcamera|ircamera|depthcamera|matchonboard|windowsbiometric|enhancedsignin|signinoptions)'
    if ($normalized -match $windowsHelloPattern) {
        return $false
    }

    $fingerprintPattern = '(fingerprint|fingerprintreader|fingerprintrreader|validitysensor|validityfingerprint|goodixfingerprint|goodixfp|synapticswbdifp|synapticsfingerprint|138afingerprint|elanwbfingerprint|elanfingerprint|fpmodule|fpsensor|authentec)'

    if ($normalized -match $fingerprintPattern) {
        return $true
    }

    if ($PnpClass -eq 'Biometric' -and $normalized -match 'fingerprint') {
        return $true
    }
...
function Test-IsExcludedWlanDevice {
    param(
        [string]$Normalized,
        [string]$Name = '',
        [string]$Description = ''
    ...
    $combined = Get-NormalizedText -Text ("{0} {1} {2}" -f $Name, $Description, $Normalized)

    return $combined -match '(wifidirect|directvirtual|virtualadapter|wanminiport|hostednetwork|bluetoothdevicepersonalareanetwork|wirelessbutton|wirelessradiocontrols|manageability|vpnadapter|microsoftvirtualethernet|miniportdriver)'
}
...
function Get-DellDeviceFeatureMatch {
    ...
    foreach ($device in $Devices) {
        ...
        switch ($FeatureName) {
            ...
            'WLAN' {
                if (Test-IsExcludedWlanDevice -Normalized $normalized -Name $name -Description $description) {
                    break
                }

                if (
                    ($pnpClass -eq 'Net' -or [string]::IsNullOrWhiteSpace($pnpClass)) -and
                    (
                        $normalized -match '(wirelesslan|wi-?fi|wifi|wlan|80211|wirelessac|wirelessax)' -or
                        $normalized -match 'wireless'
                    )
                ) {
                    return [pscustomobject]@{
                        Present = $true
            ...
            'KEYBOARD BACKLIGHT' {
                if ($normalized -match '(keyboardbacklight|kbdbacklight|kbbacklight|backlitkeyboard|keyboardillumination|kbillumination|illuminatedkeyboard)') {
                    return [pscustomobject]@{
                        Present = $true
                        Detail = $detail
                    }
            ...
            'FINGERPRINT' {
                if (Test-FingerprintPnpDevice -Text $normalized -PnpClass $pnpClass) {
                    return [pscustomobject]@{
                        Present = $true
                        Detail = $detail
                    }
            ...
            'PRIVACY' {
                if (
                    $normalized -match '(sureview|privacyscreen|privacyfilter|privacypanel|hpprivacy|integratedprivacy|privacyswitch)' -or
                    ($pnpClass -eq 'Monitor' -and $normalized -match 'privacy')
                ) {
                    return [pscustomobject]@{
                        Present = $true
                        Detail = $detail
            ...
            'RGB' {
                if (
                    $normalized -match '(rgbkeyboard|perkeyrgb|keyboardrgb|kbrgb|rgblight|mysticlight|steelseriesrgb|chromakeyboard|aurargb|perkey)' -or
                    ($normalized -match 'rgb' -and $normalized -match 'keyboard')
                ) {
                    return [pscustomobject]@{
                        Present = $true
                        Detail = $detail
...
function Invoke-GenericOemSection {
    param(
        [Parameter(Mandatory = $true)]
        $Data,
    ...
    foreach ($entry in $featureMap.GetEnumerator()) {
        $detections[$entry.Key] = Get-DellDeviceFeatureMatch -Devices $devices -FeatureName $entry.Value
    }

    Write-SectionHeader -Title "Device Manager Component Check ($Manufacturer)"
    Write-CommonSystemInfo -Data $Data
    Write-Output ""
    Write-SectionHeader -Title "Detected Components"

    foreach ($label in $featureMap.Keys) {
        $match = $detections[$label]
        $checkMark = if ($match.Present) { '[X]' } else { '[ ]' }
        $status = if ($match.Present) { 'Yes' } else { 'No' }
        Write-Output ("{0} {1} : {2}" -f $checkMark, $label, $status)

        if ($match.Present -and $match.Detail -and $match.Detail -ne 'No matching device found') {
            Write-Output ("     Detail: {0}" -f $match.Detail)
        }
    }

    Write-Output ""
    Write-SectionHeader -Title "Component Summary"
    $summary = foreach ($label in $featureMap.Keys) {
        $match = $detections[$label]
        [pscustomobject]@{
            Feature = $label
            'Device Manager' = if ($match.Present) { 'Yes' } else { 'No' }
            Detail = $match.Detail
        }
    }
...
try {
    ...
    elseif ($manufacturer -match '(?i)dell') {
        ...
        Invoke-GenericOemSection -Data $data -Manufacturer $manufacturer
    }
