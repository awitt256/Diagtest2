...
class CycleKey:
    def __init__(self, master, key_id, label, width=6, height=None, font=None):
        self.key_id = key_id
        self.label = label
...
class KeyboardGUI:
    def __init__(self, root):
        ...
        # Embedded in MYWINTEST hardware screen — use slightly smaller keys so full layout fits
        try:
            self._embedded = type(root) is not tk.Tk
        except Exception:
            self._embedded = False
        self._key_font = ("Helvetica", 9, "bold") if self._embedded else ("Helvetica", 10, "bold")
        self._key_pad = 1 if self._embedded else 2
        ...
        # Key log (matches KeyboardTesterGUI2 behavior)
        self.key_log_var = tk.StringVar(value="Key log: ")
        self.key_log_label = tk.Label(root, textvariable=self.key_log_var, bg="#001f3f", fg="#ffffff",
                                      anchor="w", justify="left", font=("Helvetica", 10, "bold"))
        self.key_log_label.pack(fill="x", padx=8, pady=(0, 4))
    ...
    def _press_by_id(self, key_id):
        try:
            ...
            if key:
                key.press()
                self._append_key_log(getattr(key, 'label', key_id))
        except Exception:
            pass
    ...
    def _append_key_log(self, key_text):
        """Append one key label to the key log line."""
        try:
            current_log = self.key_log_var.get().replace("Key log: ", "", 1).strip()
            updated_log = f"{current_log} {key_text}".strip() if current_log else str(key_text)
    ...
    def _add_key(self, container, key_id, label, **grid_opts):
        ...
        if getattr(self, "_embedded", False) and isinstance(width, int) and width > 3:
            width = max(4, int(round(width * 0.9)))
        pad = getattr(self, "_key_pad", 2)
        if "padx" not in grid_opts:
            grid_opts["padx"] = pad
        if "pady" not in grid_opts:
            grid_opts["pady"] = pad
        key = CycleKey(
            container, key_id, label,
            width=width, height=height,
            font=getattr(self, "_key_font", None),
        )
        key.grid(**grid_opts)
    ...
    def _build_layout(self):
        ...
        _cluster_pad = (8, 4) if getattr(self, "_embedded", False) else (12, 6)
        r = 0
        ...
        top_right_frame.grid(row=0, column=1, sticky="n", padx=_cluster_pad, pady=(6,0))
        ...
        mid_frame.grid(row=1, column=1, rowspan=5, sticky="n", padx=_cluster_pad)
        ...
        num_frame.grid(row=1, column=2, rowspan=5, sticky="ne", padx=(_cluster_pad[0], 0))
    ...
    def _on_keypress(self, event):
        ...
        # If the keysym indicates a keypad key (KP_*), preserve that mapping.
        key_id = ks if ks and ks.startswith('KP_') else None

        # Windows fallback: map numpad virtual-key keycodes explicitly.
        # This handles cases where keysym/char look like top-row digits.
        if key_id is None:
            kc = getattr(event, 'keycode', None)
            kp_by_keycode = {
            ...
            if kc in kp_by_keycode:
                key_id = kp_by_keycode[kc]

        # Prefer event.char for printable characters (handles punctuation)
        ch = getattr(event, 'char', '')
        # Only treat event.char as printable when no keypad mapping was chosen.
        # (also avoids control characters like Escape '\x1b' overriding keysym)
        if key_id is None and ch and ch != '\x00' and ch.isprintable():
