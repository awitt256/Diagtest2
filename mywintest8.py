...
def show_hardware_test_screen():
    ...
    ctk.CTkLabel(top_bar, text="Hardware Test Suite — Revision 0.08: got audio changer working added activation check section ",
    ...
    # Start audiog.ps1 concurrently while System Info runs
    ...
    # Run activation check on card creation and show output
    act_output = ctk.CTkTextbox(act_card, wrap="none", height=6)
    act_output.pack(fill="x", padx=14, pady=(6, 10))
    act_output.configure(state="disabled")
    ...
    def _display_activation(text):
        try:
            act_output.configure(state="normal")
            act_output.delete("1.0", "end")
            act_output.insert("1.0", text)
            act_output.configure(state="disabled")
        except Exception:
            pass
