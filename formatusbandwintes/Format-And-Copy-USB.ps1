...
try {
    ...
    # Source directory - use the directory this script is in
    ...
    Write-Host "Script name (will be excluded): $ScriptFileName" -ForegroundColor Yellow
    ...
    # Get list of available drives (D: to O:)
    $DrivesToCheck = @("D", "E", "F", "G", "H", "I", "J", "K", "L", "M", "N", "O")
    $AvailableDrives = @()
    
    Write-Host "`n=== USB Drive Scanner ===" -ForegroundColor Cyan
    Write-Host "Scanning for available drives D: - O:...`n"
    ...
    foreach ($Drive in $AvailableDrives) {
        ...
        try {
            ...
            $ExcludeFiles = @($ScriptFileName, "Format-And-Copy-USB.bat")
