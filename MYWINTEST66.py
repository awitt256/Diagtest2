...
# Set minimum size but allow full screen
app.minsize(800, 600)
...
# ------------------------------------------------------------------
# System Info Cache (prevents lag from repeated wmic calls)
# ------------------------------------------------------------------
_system_info_cache = {}
...
def _get_system_info_worker():
    """Worker function that gathers system info in background thread"""
    ...
    $sc  = Get-CimInstance Win32_PnPEntity | Where-Object { $_.Class -eq "SmartCardReader" } | Select-Object -First 1
    $scCard = Get-PnpDevice -Class SmartCard -ErrorAction SilentlyContinue | Where-Object { $_.Status -eq "OK" } | Select-Object -First 1
    [pscustomobject]@{
        CPU           = $cpu.Name
        ...
        smartcard_reader_name = $sc.Name
        smartcard_inserted    = if ($sc -and $scCard) { $true } else { $false }
        smartcard_logs        = if ($sc) { "[*] Reader detected: $($sc.Name)`n[*] Bus Status: OK`n[*] Checking for card..." } else { "[!] No smart card reader hardware found on bus." }
     } | ConvertTo-Json
    """
    ...
    try:
        ...
        if data:
            ...
            info['smartcard_reader_name'] = data.get('smartcard_reader_name') or None
            info['smartcard_inserted'] = bool(data.get('smartcard_inserted'))
            info['smartcard_logs'] = data.get('smartcard_logs', '[*] Awaiting bus poll sequence...')
    except Exception:
        pass
...
def get_system_info():
    """Gather system information for the test report (cached and threaded)"""
    ...
    if _system_info_thread is None or not _system_info_thread.is_alive():
        _system_info_thread = threading.Thread(target=_get_system_info_worker, daemon=True)
        _system_info_thread.start()

    # Return basic info immediately while thread gathers detailed info
    return {
        'computer_name': os.environ.get('COMPUTERNAME', 'N/A'),
        'username': os.environ.get('USERNAME', 'N/A'),
        'os_name': platform.system() + ' ' + platform.release(),
...
def get_system_info_sync(timeout=8):
    """Get system info synchronously (for PDF generation). Waits for background thread."""
    global _system_info_thread

    # If thread is running, wait for it
    if _system_info_thread is not None and _system_info_thread.is_alive():
        _system_info_thread.join(timeout=timeout)
    ...
    # Fallback: run synchronously if cache is empty
    return _get_system_info_worker()


# Start gathering system info immediately on import (in background)
threading.Thread(target=_get_system_info_worker, daemon=True).start()
...
def take_screenshot_and_create_pdf(status_summary=None):
    ...
    global _card_screenshots
    try:
        # Use synchronous version to wait for background thread
        sys_info = get_system_info_sync(timeout=8)
        ...
        story.append(Paragraph("Generated by DTT v0.58", footer_style))
...
def show_hardware_test_screen():
    ...
    def _set_sequence_button_idle():
        try:
            if seq_btn is not None: seq_btn.configure(text="Run Sequence", state="normal")
        except Exception:
            pass
    ...
    def _go_back():
        ...
        if _bat_after_id[0]:
            try:
                app.after_cancel(_bat_after_id[0])
            except Exception:
                pass
    ...
    # Track which card is currently highlighted
    _current_highlight_card = [None]

    def _highlight_and_show(card, color=None):
        """Highlight a card with a green border and scroll it into view."""
        try:
            # Clear previous highlight
            prev = _current_highlight_card[0]
            if prev is not None and prev is not card:
                try:
                    if widget_exists(prev):
                        # Reset color AND border width back to normal
                        prev.configure(border_color=theme_value("card_border"), border_width=0)
                except Exception:
                    pass
            _current_highlight_card[0] = card

            # Apply highlight border
            try:
                if widget_exists(card):
                    # ✅ FIX: Explicitly set border_width to 2 (or 3) so the color shows up!
                    card.configure(border_color=color or "#2ecc71", border_width=2)
            except Exception:
                pass
    ...
    def _clear_highlight():
        """Remove highlight border from the currently highlighted card."""
        try:
            prev = _current_highlight_card[0]
            if prev is not None:
                try:
                    if widget_exists(prev):
                        # ✅ FIX: Reset border width to 0 along with the border color
                        prev.configure(border_color=theme_value("card_border"), border_width=0)
                except Exception:
                    pass
            _current_highlight_card[0] = None
        except Exception:
            pass
    ...
    def _clear_highlight():
        ...
        try:
            ...
            if prev is not None:
                try:
                    if widget_exists(prev):
                        prev.configure(border_color=theme_value("card_border"))
                except Exception:
                    pass
            _current_highlight_card[0] = None
        except Exception:
    ...
    banner_path = os.path.join(BASE, "media", "dttbanner1.png")
    if os.path.exists(banner_path):
        # Load and set up the auto-resizing banner
        pil_banner = Image.open(banner_path)
        banner_img = ctk.CTkImage(light_image=pil_banner, dark_image=pil_banner, size=(1920, 150))

        banner_bg = ctk.CTkLabel(device_strip, image=banner_img, text="")
        banner_bg.place(x=0, y=0, relwidth=1, relheight=1)

        # This function handles the "Resize to Fit" logic when window is maximized
        def _handle_banner_resize(event):
            if event.width > 10 and event.height > 10:
                # Calculate dimensions to fill the area without stretching (Aspect Ratio Maintenance)
                img_w, img_h = pil_banner.size
                img_aspect = img_w / img_h
                frame_aspect = event.width / event.height

                if frame_aspect > img_aspect:
                    # Frame is wider than image aspect: match width, height will be larger (cropped)
                    new_w = event.width
                    new_h = int(event.width / img_aspect)
                else:
                    # Frame is taller than image aspect: match height, width will be larger (cropped)
                    new_h = event.height
                    new_w = int(event.height * img_aspect)

                banner_img.configure(size=(new_w, new_h))

        device_strip.bind("<Configure>", _handle_banner_resize)

    # ⏱️ Timer Overlay (Top Right of Banner)
    global _timer_label
    ...
    # Anchored to the North-East (Top Right) with 10px padding from the edges
    _timer_label.place(relx=1.0, x=-10, y=10, anchor="ne")
    ...
    sidebar = ctk.CTkFrame(main_container, width=LAYOUT["sidebar_width"], fg_color=theme_value("sidebar_bg"),
                           corner_radius=0,
    ...
    def add_sidebar_item(key, display_name, target_widget):
        ...
        lbl = ctk.CTkLabel(row, text=display_name, font=ctk.CTkFont(size=13), text_color=theme_value("title"),
                           anchor="w")
        ...
        # Hover effects
        def on_enter(e):
            row.configure(fg_color=theme_value("sidebar_hover"))
    ...
    def card(parent, title, track_key=None):
        ...
        header_strip = ctk.CTkFrame(f, fg_color=theme_value("header_bg"), corner_radius=10,
    ...
    def _apply_relative_card_width(widget, ratio):
        try:
            widget.pack_configure(fill="none", anchor="w")
        except Exception:
            pass

        def _update_width(event=None):
            try:
                parent = widget.nametowidget(widget.winfo_parent())
                available_width = parent.winfo_width()
                if available_width <= 1:
                    available_width = app.winfo_width()
                if available_width <= 1:
                    available_width = 850
                widget.configure(width=max(220, int(available_width * ratio)))
            except Exception:
                pass
    ...
    def _apply_theme(theme_name):
        ...
        for widget in themed_text_widgets:
            try:
                widget.configure(bg=theme_value("embed_bg"), fg=theme_value("embed_text"),
                                 insertbackground=theme_value("embed_text"))
            except Exception:
                pass
        ...
        # Restyle all embedded hosts to match new theme
        for host in (bat_host, tp_host, mic_embed_host, kb_embed_host):
            try:
                if widget_exists(host):
                    _restyle_tk_tree(host)
            except Exception:
                pass
        ...
        try:
            ...
            _draw_brightness_bar(
                int(str(brightness_value.cget("text")).replace("Current:", "").replace("%", "").strip() or "0"))
        except Exception:
            pass
    ...
    def _select_operator(name, color):
        ...
        def _force_start_audio():
            print("[DEBUG] _force_start_audio called")
            try:
                if hasattr(operator_card, 'set_pass'):
                    operator_card.set_pass()
                    print("[DEBUG] Set operator_card to pass")
            except Exception as e:
                print(f"[DEBUG] Error setting pass: {e}")
            # Directly run the audio script without any card infrastructure
            print("[DEBUG] Running audio script directly")
    ...
    # Clean up header strip built-in child elements to avoid rendering duplicates
    try:
    ...
    def _start_audiog():
        # Bypass logic to prevent concurrency stalls
        if ag_is_running[0]: return
        ag_is_running[0] = True

        # Force the visual label text state to register running mode instantly
        ui_call(lambda: ag_status_label.configure(text="Running Audio Changer...", text_color="#58a6ff"))

        # Support absolute cross-compatible case variation checks for the target path
        ...
        if not os.path.exists(script_path):
            ...
            # Inject standard fallback attributes to force unblock step sequencer
            for attr in ['status', '_status', 'current_status']:
                if hasattr(ag_card, attr): setattr(ag_card, attr, "fail")
            ui_call(lambda: ag_card.set_fail() if hasattr(ag_card, 'set_fail') else None)
            ...
            def _reader():
                try:
                    while True:
                        ...
                        if line:
                            ui_call(lambda l=line: _add_ag_line(l))
                        else:
                            time.sleep(0.1)
                finally:
                    rc = proc.poll() or 0

                    # Render runtime results label visually
                    ui_call(lambda: ag_status_label.configure(text="✓ Done" if rc == 0 else "✗ Failed",
                                                              text_color="#7ee787" if rc == 0 else "#ff7b72"))

                    # ══════════════════════════════════════════════════════════════════
                    # FORCE DIRECT AUTOMATION STATE INJECTION
                    # ══════════════════════════════════════════════════════════════════
                    state_str = "pass" if rc == 0 else "fail"

                    # Directly assign tracking status string fields to break _wait_for_card_result checking loop
                    for attr in ['status', '_status', 'current_status']:
                        if hasattr(ag_card, attr):
                            setattr(ag_card, attr, state_str)

                    # Trigger internal custom function handlers if mapped by your wrapper framework
                    if rc == 0 and hasattr(ag_card, 'set_pass'):
                        ui_call(lambda: ag_card.set_pass())
                    elif rc != 0 and hasattr(ag_card, 'set_fail'):
                        ui_call(lambda: ag_card.set_fail())

                    # Set multi-threaded notification primitive events if explicitly held inside card context
                    if hasattr(ag_card, '_sequence_done_event') and ag_card._sequence_done_event:
                        try:
                            ag_card._sequence_done_event.set()
                        except Exception:
                            pass

                    # Hand execution focus cleanly back to the automated sequence loop manager
                    ag_is_running[0] = False
    ...
    def _run_audiog_clicked():
        print("[DEBUG] _run_audiog_clicked called")
        ...
        print("[DEBUG] Starting thread for _start_audiog")
        threading.Thread(target=_start_audiog, daemon=True).start()
    ...
    try:
        ...
        header_row.pack(fill="x", padx=14, pady=(10, 6))
        ctk.CTkLabel(header_row, text="🖥️  System Info", font=ctk.CTkFont(size=14, weight="bold"),
                     text_color="#58a6ff").pack(side="left")
        try:
            ctk.CTkButton(header_row, text="⟳", width=28, height=28, fg_color="#444444", hover_color="#555555",
                          command=_start_system_info_once).pack(side="right")
        except Exception:
            pass
    except Exception:
        pass
    ...
    _bios_sku_base = [None]  # sku minus last 4 chars (no #ABA suffix)
    _bios_serial_full = [None]  # full serial string from System Info
    _bios_sku_full = [None]  # full sku string from System Info (no #ABA suffix)
    _bios_sku_suffix = [None]  # the #ABA-style suffix, if present (without the leading #)
    ...
    _form_serial_matched = [False]
    _form_sku_matched = [False]
    ...
    def _update_form_serial(serial_full):
        ...
        try:
            ...
            _bios_serial_full[0] = serial_full
    ...
    def _update_form_sku(sku_full):
        ...
        try:
            ...
            # Split off trailing model suffix like #ABA if present
            suffix = None
            if "#" in sku_full:
                parts = sku_full.split("#", 1)
                sku_full = parts[0].strip()
                suffix = parts[1].strip() if len(parts) > 1 else None
            _bios_sku_full[0] = sku_full
            _bios_sku_suffix[0] = suffix
            if suffix is not None:
                # HP-style SKU (has a #ABA-type suffix): show the first 5
                # characters, tech types the remaining tail (e.g. last 2)
                # either bare or with the #suffix appended.
                base = sku_full[:5]
                hint = "  ← Add last {} digits".format(max(len(sku_full) - len(base), 0))
            else:
                # Non-HP SKU: show all but the last 4, tech types the last 4.
                if len(sku_full) > 4:
                    base = sku_full[:-4]
                else:
                    base = sku_full
                hint = "  ← Add last 4 digits"
    ...
    def _reset_form_fields():
        """Reset the Serial/SKU check-label entries back to the
        'Waiting for System Info...' state, clearing any matched/typed
        values. Called on card reset and on global refresh."""
        try:
            _bios_serial_base[0] = None
            _bios_serial_full[0] = None
            _form_serial_matched[0] = False
            _form_serial_placeholder_active[0] = False
            entry = _form_serial_entry[0]
            if entry is not None:
                entry.configure(state="normal", fg="#000000", bg=theme_value("card_bg"))
                entry.delete(0, "end")
                entry.insert(0, "Waiting for System Info...")
                entry.configure(state="disabled")
        except Exception:
            pass
        try:
            _bios_sku_base[0] = None
            _bios_sku_full[0] = None
            _bios_sku_suffix[0] = None
            _form_sku_matched[0] = False
                                ...
                                for comp_key, keys in keyword_map.items():
                                    for k in keys:
                                        if k in low_line:
                                            ...
                                            if any(tok in low_line for tok in
                                                   (' yes', ': yes', 'enabled', 'present', 'true', 'available')):
                                                set_comp(comp_key, True)
                                            elif any(tok in low_line for tok in
                                                     (' no', ': no', 'disabled', 'not present', 'false',
                                                      'unavailable')):
    ...
    try:
        ...
        _register_title_label(ctk.CTkLabel(_form_header_row, text="📋  Form",
                                           ...
                                           text_color="#58a6ff")).pack(side="left")
        try:
            _register_refresh_button(ctk.CTkButton(_form_header_row, text="⟳", width=28, height=28, fg_color="#444444",
                                                   hover_color="#555555", command=_start_system_info_once)).pack(
                side="right")
        except Exception:
            pass
    except Exception:
    ...
    try:
        ...
        check_label_frame = _register_ctk_frame(
        ...
        serial_entry_frame = _register_ctk_frame(
            ctk.CTkFrame(serial_col, fg_color=theme_value("card_bg"), corner_radius=6, border_width=1,
                         border_color="#3a3f4a"))
        serial_entry_frame.pack(fill="x")
        _serial_tk_entry = _register_tk_frame(tk.Entry(
            serial_entry_frame, font=("Segoe UI", 11),
            fg="#000000", bg=theme_value("card_bg"),
        ...
        sku_entry_frame = _register_ctk_frame(
            ctk.CTkFrame(sku_col, fg_color=theme_value("card_bg"), corner_radius=6, border_width=1,
                         border_color="#3a3f4a"))
        ...
        def _serial_check_match():
            try:
                entry = _form_serial_entry[0]
                if entry is None:
                    return
                full = _bios_serial_full[0] or ""
                ...
                if full and typed and typed.upper() == full.upper():
                    if not _form_serial_matched[0]:
                        ...
                        # Optional: If both Serial and SKU are matched, auto-pass the card
                    ...
                    if _form_serial_matched[0]:
                        _form_serial_matched[0] = False
                    entry.configure(bg=theme_value("card_bg"), fg="#000000")
            except Exception:
                pass
        ...
        def _sku_max_len():
            ...
            try:
                ...
                suffix = _bios_sku_suffix[0]
                if suffix is not None:
                    tail_len = max(len(full) - len(base), 0)
                    return len(base) + tail_len + 1 + len(suffix)
                return len(base) + 4
            except Exception:
                return len(_bios_sku_base[0] or "") + 4

        def _sku_check_match():
            try:
                entry = _form_sku_entry[0]
                if entry is None:
                    return
                full_plain = (_bios_sku_full[0] or "").strip()
                suffix = _bios_sku_suffix[0]
                full_with_suffix = f"{full_plain}#{suffix}" if suffix else None
                typed = entry.get().strip()
                is_match = False
                if typed:
                    if full_plain and typed.upper() == full_plain.upper():
                        is_match = True
                    elif full_with_suffix and typed.upper() == full_with_suffix.upper():
                        is_match = True
                if is_match:
                    if not _form_sku_matched[0]:
                        ...
                        # If both are now matched, auto-pass the card
                        if _form_serial_matched[0]:
                            try:
                                comp_card.set_pass()
                            except Exception:
        ...
        def _make_color_swatch(parent, label, bg_hex, fg_hex):
            ...
            # every theme refresh, which was wiping out the real swatch
            # colors (black/white/red/etc.) and replacing them all with
            # the same pale theme background.
            ...
            def _select(l=label, outer=swatch_outer, dot=color_dot, bg=bg_hex):
                _selected_color[0] = l
                # Clear selection borders from all swatches first
                for ref in _color_btn_refs.values():
                    try:
                        ref.configure(border_width=0)
                    except Exception:
                        pass

                # Apply a green highlight border to show this swatch is selected
                try:
                    outer.configure(border_width=2, border_color="#2ecc8a")
                except Exception:
                    pass
    ...
    _apply_relative_card_width(net_card, 0.33)
    try:
        net_card.pack_configure(fill="none", padx=14, pady=8)
    except Exception:
        pass
    try:
        try:
    ...
    wifi_networks_frame = _register_ctk_frame(
        ctk.CTkFrame(net_content, fg_color=theme_value("embed_bg"), corner_radius=8))
    wifi_networks_frame.pack(fill="x", padx=4, pady=(8, 4))
    ...
    # Simple label-based list for networks (like adapter section)
    wifi_networks_status = _register_subtle_label(ctk.CTkLabel(
        wifi_networks_frame,
    ...
    wifi_networks_status.pack(anchor="w", padx=10, pady=(2, 4))

    # Container for network labels
    wifi_networks_list = ctk.CTkFrame(wifi_networks_frame, fg_color="transparent")
    wifi_networks_list.pack(fill="x", padx=10, pady=(0, 10))

    # Store references for updates
    net_sections["wifi_networks_status"] = wifi_networks_status
    net_sections["wifi_networks_list"] = wifi_networks_list
    net_sections["wifi_network_labels"] = []
    ...
    def _scan_wifi_networks():
        """Scan for available WiFi networks and populate the label list"""
        wifi_networks_status = net_sections.get("wifi_networks_status")
        wifi_networks_list = net_sections.get("wifi_networks_list")
        wifi_network_labels = net_sections.get("wifi_network_labels", [])

        if not wifi_networks_status or not widget_exists(wifi_networks_status):
            return
        if not wifi_networks_list or not widget_exists(wifi_networks_list):
            return

        # Clear existing network labels
        for lbl in wifi_network_labels:
            try:
                lbl.destroy()
            except Exception:
                pass
        wifi_network_labels.clear()

        # Update status to scanning
        wifi_networks_status.configure(text="Scanning for networks...", text_color="#FF9800")
        ...
        try:
            ...
            if "location permission" in output.lower():
                ...
                try:
                    ...
                    """
                    subprocess.run(["powershell", "-Command", enable_cmd], capture_output=True, timeout=5)
                    # Retry scan
                    result = subprocess.run(
                        ["cmd", "/c", "netsh", "wlan", "show", "networks"],
                        capture_output=True,
            ...
            # Filter to only show ISPTEK_GUEST and ISPTEK_OFFICE networks
            allowed_ssids = {'ISPTEK_GUEST', 'ISPTEK_OFFICE'}

            for i, line in enumerate(lines):
                line_stripped = line.strip()

                # Look for SSID line - format is "SSID 1 : NetworkName" (NOT "BSSID")
                if re.match(r'^SSID\s+\d+\s*:', line_stripped, re.IGNORECASE):
                    if current_network:
                    ...
                    if match:
                        ...
                        # Only include ISPTEK networks
                        if ssid and ssid.upper() in allowed_ssids:
            ...
            # Remove duplicates and filter to only ISPTEK networks
            unique_networks = []
            seen_ssids = set()
            for net in networks:
                ssid = net.get("ssid", "").strip()
                # Only include ISPTEK_GUEST and ISPTEK_OFFICE
                if ssid and ssid.upper() in allowed_ssids:
            ...
            def get_signal(net):
                try:
                    return int(net.get("signal", "0%").replace("%", ""))
                except:
                    return 0

            unique_networks.sort(key=get_signal, reverse=True)

            # Update status
            if unique_networks:
                wifi_networks_status.configure(
                    text=f"Found {len(unique_networks)} network(s):",
                    text_color="#2ecc71"
                )
                ...
                for network in unique_networks:
                    ...
                    network_text = f"{signal_icon}  {ssid}  •  Signal: {signal}"
                    ...
                    net_label.pack(anchor="w", padx=0, pady=2)
                    wifi_network_labels.append(net_label)
            else:
                wifi_networks_status.configure(text="No ISPTEK networks found", text_color="#ff7b72")

        except Exception as e:
            print(f"WiFi network scan error: {e}")
            wifi_networks_status.configure(text=f"Scan failed: {str(e)}", text_color="#ff7b72")
    ...
    # AUTO MOVE TO MEMORY WHEN NETWORK FINISHES
    def _on_net_marked():
        """After Network PASS/FAIL, show Memory card and run RAM audit."""

        def _show_and_start_ram():
            try:
                _highlight_and_show(ram_card)
            except Exception:
                pass
            try:
        ...
        if hasattr(net_card, 'set_pass') and hasattr(net_card, 'pass_btn'):
            ...
            def _net_pass():
                ...
                if _sequence_running[0]:
                    ui_call(lambda: _highlight_and_show(ram_card))
                    return
                _on_net_marked()
    ...
    def _on_components_marked():
        """After Form PASS/FAIL, show Network card and run adapter check."""
    ...
    # ══════════════════════════════════════════════════════════════════
    # 3C. MEMORY (RAM) AUDIT CARD
    # ══════════════════════════════════════════════════════════════════
    ram_card = card(body, "🧠  Memory", track_key="ram")
    _apply_relative_card_width(ram_card, 0.67)
    try:
        ram_card.pack_configure(fill="none", padx=14, pady=(LAYOUT["card_pad_y"], 0))
    except Exception:
        pass
    try:
        try:
            ram_card.winfo_children()[0].destroy()
        except Exception:
            pass
    except Exception:
        pass

    ram_header_row = ctk.CTkFrame(ram_card, fg_color="transparent")
    ram_header_row.pack(fill="x", padx=14, pady=(10, 6))
    _register_title_label(ctk.CTkLabel(
        ram_header_row,
        text="🧠  Memory",
        font=ctk.CTkFont(size=14, weight="bold"),
    ...
    ram_content = ctk.CTkFrame(ram_card, fg_color="transparent")
    ram_content.pack(fill="x", expand=False, padx=14, pady=(2, 12))

    ram_total_lbl = _register_subtle_label(ctk.CTkLabel(
        ram_content,
        text="Total System Memory: Run refresh or sequencer to check.",
        font=ctk.CTkFont(size=12, weight="bold"),
        text_color="#9fb3c8",
        justify="left"
    ))
    ram_total_lbl.pack(anchor="w", pady=(0, 8))

    ram_speed_banner = ctk.CTkFrame(ram_content, height=36, fg_color="#2c3e50", corner_radius=8)
    ram_speed_banner.pack(fill="x", pady=(0, 4))
    ram_speed_banner.pack_propagate(False)
    ram_speed_lbl = ctk.CTkLabel(
        ram_speed_banner,
        text="Speed Sync: Auditing...",
        font=ctk.CTkFont(size=12, weight="bold"),
        text_color="#ffffff"
    )
    ram_speed_lbl.pack(expand=True)

    ram_timing_banner = ctk.CTkFrame(ram_content, height=36, fg_color="#2c3e50", corner_radius=8)
    ram_timing_banner.pack(fill="x", pady=(0, 8))
    ram_timing_banner.pack_propagate(False)
    ram_timing_lbl = ctk.CTkLabel(
        ram_timing_banner,
        text="Timing / Voltage Layout: Auditing...",
        font=ctk.CTkFont(size=12, weight="bold"),
        text_color="#ffffff"
    )
    ram_timing_lbl.pack(expand=True)

    _register_subtle_label(ctk.CTkLabel(
        ram_content,
        text="Detected Physical Modules Mapping:",
        font=ctk.CTkFont(size=12, weight="bold"),
        text_color="#8b949e",
        justify="left"
    )).pack(anchor="w", pady=(0, 2))

    ram_details_box = ctk.CTkTextbox(
        ram_content,
        height=140,
    ...
    ram_details_box.pack(fill="x", pady=(0, 0))
    ram_details_box.insert("1.0", "Run refresh or sequencer to scan installed memory modules...")
    ram_details_box.configure(state="disabled")

    def _refresh_ram_textbox(text):
        if not widget_exists(ram_details_box):
            return
        ram_details_box.configure(state="normal")
        ram_details_box.delete("1.0", "end")
        ram_details_box.insert("1.0", text)
        ram_details_box.configure(state="disabled")

    def _get_physical_memory_data():
        """Query installed RAM modules via PowerShell CIM (no extra deps needed)."""
        ps_cmd = (
        ...
        for line in (result.stdout or "").splitlines():
            ...
            while len(parts) < 6:
                parts.append("")
            locator, capacity, speed, configured_speed, mfg, part = parts[:6]
            modules.append({
                "locator": locator.strip() or "Unknown Slot",
                "capacity": capacity.strip(),
                "speed": speed.strip(),
                "configured_speed": configured_speed.strip(),
                "manufacturer": (mfg.strip() or "Unknown Vendor"),
                "part": (part.strip() or "Unknown Part"),
            })
        return modules

    def _ram_refresh(auto_pass_fail=True):
        """Audit installed RAM modules for capacity, speed and timing/vendor mismatches."""
        try:
            modules = _get_physical_memory_data()
        except Exception as e:
            def _apply_error():
                if not widget_exists(ram_card):
                    return
                ram_total_lbl.configure(text="Total System Memory: Error", text_color="#ff7b72")
                _refresh_ram_textbox(f"[-] RAM audit error: {e}")
                if auto_pass_fail:
                    try:
                        if hasattr(ram_card, "set_fail"):
                            ram_card.set_fail()
                    except Exception:
                        pass
        ...
        if not modules:
            def _apply_none():
                if not widget_exists(ram_card):
                    return
                ram_total_lbl.configure(text="Total System Memory: Error", text_color="#ff7b72")
                _refresh_ram_textbox("[-] No physical memory modules reported by motherboard context.")
                if auto_pass_fail:
                    try:
                        if hasattr(ram_card, "set_fail"):
                            ram_card.set_fail()
        ...
        for idx, mod in enumerate(modules):
            try:
                capacity_bytes = int(mod["capacity"]) if mod["capacity"].isdigit() else 0
            except Exception:
                capacity_bytes = 0
            total_bytes += capacity_bytes
            cap_gb = int(capacity_bytes / (1024 ** 3)) if capacity_bytes else 0

            speed = 0
            try:
                if mod["speed"].isdigit() and int(mod["speed"]) > 0:
                    speed = int(mod["speed"])
                elif mod["configured_speed"].isdigit():
                    speed = int(mod["configured_speed"])
            except Exception:
                speed = 0

            if speed > 0:
                speeds.append(speed)
            manufacturers.append(mod["manufacturer"].lower())
            part_numbers.append(mod["part"].lower())

            speed_display = f"{speed}MHz" if speed > 0 else "Unknown Speed"
            slot_logs.append(
                f"[{mod['locator']}] {cap_gb}GB {mod['manufacturer']}\n"
                f"    -> Speed: {speed_display} | Part: {mod['part']}\n"
            )
        ...
        if len(set(speeds)) > 1:
            speed_ok = False
            speed_status = f"⚠️ SPEED MISMATCH: Bus throttled to {min(speeds)}MHz (Max chip: {max(speeds)}MHz)"
            speed_bg = "#7e3d11"
            speed_color = "#ff944d"
        elif speeds:
        ...
        def _apply():
            if not widget_exists(ram_card):
                return
            ram_total_lbl.configure(text=f"Total System Memory: {total_gb} GB ({len(modules)} module(s))",
                                    text_color="#9fb3c8")
            ram_speed_banner.configure(fg_color=speed_bg)
            ram_speed_lbl.configure(text=speed_status, text_color=speed_color)
            ram_timing_banner.configure(fg_color=timing_bg)
            ram_timing_lbl.configure(text=timing_status, text_color=timing_color)
            _refresh_ram_textbox(log_text)

            if auto_pass_fail:
                try:
                    if speed_ok and timing_ok and hasattr(ram_card, "set_pass"):
                        ram_card.set_pass()
                    elif hasattr(ram_card, "set_fail"):
                        ram_card.set_fail()
                except Exception:
                    pass
    ...
    try:
        ctk.CTkButton(
            ...
            fg_color="#444444",
            hover_color="#555555",
            command=lambda: threading.Thread(target=_ram_refresh, daemon=True).start()
        ).pack(side="right")
    except Exception:
        pass

    # AUTO MOVE TO BATTERY WHEN MEMORY FINISHES
    def _on_ram_marked():
        """After Memory PASS/FAIL, show Battery card and run battery check."""
    ...
    try:
        if hasattr(ram_card, 'set_pass') and hasattr(ram_card, 'pass_btn'):
            _ram_orig_pass = ram_card.set_pass
            _ram_orig_fail = ram_card.set_fail
            ...
            ram_card.set_pass = _ram_pass
            ram_card.set_fail = _ram_fail
            ram_card.pass_btn.configure(command=_ram_pass)
            ram_card.fail_btn.configure(command=_ram_fail)
    except Exception:
        pass
    ...
    try:
        ...
        header_row.pack(fill="x", padx=14, pady=(10, 6))
        _register_title_label(ctk.CTkLabel(header_row, text="🔋  Battery", font=ctk.CTkFont(size=14, weight="bold"),
                                           text_color="#58a6ff")).pack(side="left")
        try:
            _register_refresh_button(
                ctk.CTkButton(header_row, text="⟳", width=28, height=28, fg_color="#444444", hover_color="#555555",
                              command=lambda: _start_battery_embed())).pack(side="right")
        except Exception:
            pass
    except Exception:
        pass
    ...
    # Status label
    bat_status = ctk.CTkLabel(bat_card, text="Click Start to load battery module.",
                              font=ctk.CTkFont(size=12), text_color="#9fb3c8")
    bat_status.pack(anchor="w", padx=14, pady=(0, 6))

    # Host frame for embedded BATTERY.py widget
    bat_host = _register_tk_frame(tk.Frame(bat_card, bg=theme_value("embed_bg")))
    bat_host.pack(fill="both", expand=True, padx=14, pady=(6, 0))

    bat_active = [False]

    def _start_battery_embed():
        """Embed BATTERY.py widget into the battery card."""
        bat_active[0] = True
        try:
            bat_status.configure(text="Battery module loading...", text_color="#7ee787")
        except Exception:
            pass

        # Clear any existing children
        for w in bat_host.winfo_children():
            try:
                w.destroy()
            except Exception:
                pass

        _bat_path = os.path.join(BASE, "BATTERY.py")
        if not os.path.exists(_bat_path):
            ctk.CTkLabel(bat_host, text=f"BATTERY.py not found: {_bat_path}", text_color="#ff7b72").pack(pady=14)
            bat_active[0] = False
            return

        try:
            # Import BATTERY module dynamically
            _bat_spec = _importlib_util.spec_from_file_location("BATTERY", _bat_path)
            battery_mod = _importlib_util.module_from_spec(_bat_spec)
            _bat_spec.loader.exec_module(battery_mod)

            # Update host background to match battery theme
            bat_host.configure(bg=battery_mod.T["bg"])

            # Create embedded battery widget inside the host frame
            battery_widget = battery_mod.BatteryWidget(embed_host=bat_host)

            # Force update to show the widget
            bat_host.update_idletasks()

            try:
                bat_status.configure(text="Battery module loaded.", text_color="#7ee787")
            except Exception:
                pass
        except Exception as e:
            import traceback
            error_msg = f"Could not embed battery module: {e}\n{traceback.format_exc()}"
            print(error_msg)
            ctk.CTkLabel(bat_host, text=f"Error: {e}", text_color="#ff7b72", wraplength=400).pack(pady=14)
            bat_active[0] = False

    def _stop_battery_embed():
        """Stop the embedded battery widget."""
        bat_active[0] = False
        for w in bat_host.winfo_children():
            try:
                w.destroy()
                ...
                pass
        try:
            bat_status.configure(text="Battery module stopped.", text_color="#9fb3c8")
        except Exception:
            pass
    ...
    try:
        ...
        header_row.pack(fill="x", padx=14, pady=(10, 6))
        ctk.CTkLabel(header_row, text="🔊  Speaker Test", font=ctk.CTkFont(size=14, weight="bold"),
                     text_color="#58a6ff").pack(side="left")
        try:
            # Use lambda so callback resolves after local funcs are defined.
            ctk.CTkButton(header_row, text="⟳", width=28, height=28, fg_color="#444444", hover_color="#555555",
                          command=lambda: _spk_play()).pack(side="right")
        except Exception:
            pass
    except Exception:
        pass
    ...
    try:
        ...
        header_row.pack(fill="x", padx=14, pady=(10, 6))
        _register_title_label(
            ctk.CTkLabel(header_row, text="🎤  Microphone Test", font=ctk.CTkFont(size=14, weight="bold"),
                         text_color="#58a6ff")).pack(side="left")
        ...
        try:
            _register_refresh_button(
                ctk.CTkButton(header_row, text="⟳", width=28, height=28, fg_color="#444444", hover_color="#555555",
                              command=lambda: _mic_refresh_clicked())).pack(side="right")
        except Exception:
            pass
    except Exception:
        pass
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                pass
            meter_style.configure('Green.Horizontal.TProgressbar', troughcolor='#1e2a3a', background='#7ee787',
                                  thickness=14)
    ...
    try:
        ...
        ctk.CTkLabel(tp_header_row, text="🖱️  Touchpad Test", font=ctk.CTkFont(size=14, weight="bold"),
                     text_color="#58a6ff").pack(side="left")
        try:
            ctk.CTkButton(tp_header_row, text="⟳", width=28, height=28, fg_color="#444444", hover_color="#555555",
                          command=lambda: _start_touchpad_embed()).pack(side="right")
        except Exception:
            pass
    except Exception:
        pass
    ...
    def _start_touchpad_embed():
        ...
        try:
            # Dynamically load TouchpadTester from touchpad_tester_3.py
            _tp_path = _os.path.join(BASE, "touchpad_tester_3.py")
            if not _os.path.exists(_tp_path):
                ctk.CTkLabel(tp_host, text=f"touchpad_tester_3.py not found: {_tp_path}", text_color="#ff7b72").pack(
                    pady=14)
                return

            _tp_spec = _importlib_util.spec_from_file_location("touchpad_tester_3", _tp_path)
    ...
    ctk.CTkButton(tp_btn_row, text="Start", width=120, height=32, command=_start_touchpad_embed).pack(side="left",
                                                                                                      padx=(0, 8))
    ...
    try:
        ...
        ctk.CTkLabel(br_header_row, text="💡  Brightness", font=ctk.CTkFont(size=14, weight="bold"),
                     text_color="#58a6ff").pack(side="left")
        try:
            ctk.CTkButton(br_header_row, text="⟳", width=28, height=28, fg_color="#444444", hover_color="#555555",
                          command=lambda: threading.Thread(target=_start_brightness_test, daemon=True).start()).pack(
                side="right")
        except Exception:
            pass
    except Exception:
        pass
    brightness_status = ctk.CTkLabel(brightness_card, text="Loading brightness info...", font=ctk.CTkFont(size=12),
                                     text_color="#9fb3c8")
    brightness_status.pack(anchor="w", padx=14, pady=(0, 6))
    brightness_value = ctk.CTkLabel(brightness_card, text="Current: --", font=ctk.CTkFont(size=18, weight="bold"),
                                    text_color="#7ee787")
    ...
    br_start_btn = ctk.CTkButton(br_btn_row, text="Start Test", width=120,
                                 command=lambda: threading.Thread(target=_start_brightness_test, daemon=True).start())
    ...
    def _draw_brightness_bar(percent):
        try:
            ...
            knob_x = pad + fill_w
            knob_r = 12
            br_canvas.create_oval(knob_x - knob_r, (h // 2) - knob_r, knob_x + knob_r, (h // 2) + knob_r,
                                  fill="#d4af37", outline="#a07e2a")
        except Exception:
            pass
    ...
    def _set_monitor_brightness(percent):
        ...
        try:
            ...
            rc1, out1 = run_process_capture(["powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", ps1],
                                            shell=False)
            if rc1 == 0:
                return True
            ...
            rc2, out2 = run_process_capture(["powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", ps2],
                                            shell=False)
            if rc2 == 0:
                return True
            ...
            rc3, out3 = run_process_capture(["powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", ps3],
                                            shell=False)
            if rc3 == 0:
                return True
            ...
            try:
                ui_call(lambda: brightness_status.configure(text=f"Brightness command failed (codes {rc1},{rc2},{rc3})",
                                                            text_color="#ff7b72"))
            except Exception:
                pass
            return False
    ...
    def _start_brightness_test():
        ...
        def worker():
            try:
                ...
                rc, out = run_process_capture(
                    ["powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", ps_cmd], shell=False)
                if rc != 0 or not out.strip():
                    ui_call(lambda: brightness_status.configure(text="Could not read current brightness.",
                                                                text_color="#ff7b72"))
                    return
                try:
                    ...
                    ui_call(
                        lambda: brightness_status.configure(text="Could not parse brightness.", text_color="#ff7b72"))
                    return
                ...
                if not _br_test_stop.is_set():
                    ...
                    ui_call(
                        lambda: brightness_status.configure(text="Brightness cycle complete.", text_color="#7ee787"))
                else:
    ...
    def _initial_draw():
        try:
            ...
            ui_call(
                lambda: brightness_status.configure(text="Could not initialize brightness UI.", text_color="#ff7b72"))
    ...
    # --- SMART CARD CARD LAYOUT (ALIGNED WITH SCREADER2 DESIGN) ---
    sc_hw_frame = ctk.CTkFrame(smartcard_card, height=45, fg_color="#2c3e50")
    sc_hw_frame.pack(fill="x", padx=15, pady=4)
    sc_hw_frame.pack_propagate(False)
    sc_hw_label = ctk.CTkLabel(sc_hw_frame, text="Reader Hardware: Initializing...",
                               font=ctk.CTkFont(size=12, weight="bold"))
    sc_hw_label.pack(expand=True)

    sc_card_frame = ctk.CTkFrame(smartcard_card, height=45, fg_color="#2c3e50")
    sc_card_frame.pack(fill="x", padx=15, pady=4)
    sc_card_frame.pack_propagate(False)
    sc_card_label = ctk.CTkLabel(sc_card_frame, text="Card Presence: Waiting...",
                                 font=ctk.CTkFont(size=12, weight="bold"))
    sc_card_label.pack(expand=True)

    sc_details_box = ctk.CTkTextbox(smartcard_card, height=70, font=ctk.CTkFont(family="Consolas", size=10),
                                    fg_color="#0d1117", border_color="#21262d", border_width=1)
    sc_details_box.pack(fill="x", padx=15, pady=(4, 10))

    def _sc_refresh_from_cache(sys_info):
        # Read keys written by the background execution thread
        reader_hardware_name = sys_info.get('smartcard_reader_name', None)
        card_inserted = sys_info.get('smartcard_inserted', False)
        log_messages = sys_info.get('smartcard_logs', '[*] Awaiting bus poll sequence...')

        # Update logs box
        sc_details_box.configure(state="normal")
        sc_details_box.delete("1.0", "end")
        sc_details_box.insert("1.0", log_messages)
        sc_details_box.configure(state="disabled")

        # 1. Reader Hardware Row Visual State
        if reader_hardware_name:
            display_name = reader_hardware_name if len(reader_hardware_name) <= 35 else reader_hardware_name[
                                                                                            :32] + "..."
            sc_hw_frame.configure(fg_color="#14341c")  # Pass Green
            sc_hw_label.configure(text=f"READER: {display_name.upper()}", text_color="#52c41a")

            # 2. Card Presence Row Visual State (Only matters if reader exists)
            if card_inserted:
                sc_card_frame.configure(fg_color="#14341c")
                sc_card_label.configure(text="💳 CARD DETECTED IN SLOT", text_color="#52c41a")
            else:
                sc_card_frame.configure(fg_color="#3A2222")  # Warning Red
                sc_card_label.configure(text="📥 SLOT EMPTY - INSERT CARD", text_color="#E74C3C")
        else:
            sc_hw_frame.configure(fg_color="#3A2222")
            sc_hw_label.configure(text="READER: NOT DETECTED", text_color="#E74C3C")
            sc_card_frame.configure(fg_color="#2c3e50")
            sc_card_label.configure(text="CARD STATUS: N/A (NO READER)", text_color="#7F8C8D")

    def _sc_refresh_logic():
        # Refresh system info cache then update widgets
        threading.Thread(target=lambda: (get_system_info_sync(timeout=2),
                                         ui_call(lambda: _sc_refresh_from_cache(get_system_info()))),
                         daemon=True).start()

    # Alias used by the refresh button and the auto-advance sequencer.
    # The card is fully embedded (no external SCREADER2.PY widget needed) -
    # refreshing it just re-runs the same live-status logic.
    ...
    ctk.CTkLabel(fp_header, text="👆  Fingerprint Reader", font=ctk.CTkFont(size=14, weight="bold"),
                 text_color="#58a6ff").pack(side="left")
    ctk.CTkButton(fp_header, text="⟳", width=28, height=28, fg_color="#444444", hover_color="#555555",
                  command=lambda: threading.Thread(target=_fingerprint_refresh, daemon=True).start()).pack(side="right")

    fp_status_lbl = ctk.CTkLabel(fingerprint_card, text="Checking for fingerprint readers...",
                                 font=ctk.CTkFont(size=12), text_color="#9fb3c8")
    ...
    fp_svc = _register_subtle_label(
        ctk.CTkLabel(fp_info, text="Service: --", font=ctk.CTkFont(size=11), text_color="#9fb3c8"))
    fp_svc.pack(anchor="w", padx=12, pady=(10, 4))
    fp_dev = _register_subtle_label(
        ctk.CTkLabel(fp_info, text="Biometric: -- | Fingerprint: --", font=ctk.CTkFont(size=11), text_color="#9fb3c8"))
    fp_dev.pack(anchor="w", padx=12, pady=(4, 10))
    fp_txt = _register_text_widget(
        tk.Text(fp_info, bg=theme_value("embed_bg"), fg=theme_value("embed_text"), font=("Consolas", 10), height=6,
                wrap="word", borderwidth=0, highlightthickness=0))
    ...
    def _fingerprint_refresh():
        ...
        try:
            ...
            for line in out.split('\n'):
                line = line.strip()
                if line.startswith("BIO_SERVICE_STATUS:"):
                    svc = line.split(":", 1)[1].strip()
                elif line.startswith("TOTAL_BIOMETRIC_DEVICES:"):
                    try:
                        bio = int(line.split(":", 1)[1].strip())
                    except:
                        bio = 0
                    ...
                    try:
                        ...
                        wrk = 0
                elif line.startswith("WINDOWS_HELLO_AVAILABLE:"):
                    hello = ("True" in line)
                elif line.startswith("TEST_RESULT:"):
                    res = line.split(":", 1)[1].strip()
                elif line.startswith("TEST_MESSAGE:"):
            ...
            def _upd():
                fp_status_lbl.configure(text=f"Fingerprint: {res}",
                                        text_color="#7ee787" if res == "PASS" else "#ff7b72")
                fp_svc.configure(text=f"Service: {svc}")
                fp_dev.configure(text=f"Biometric: {bio} | Fingerprint: {fp_r}")
                fp_txt.configure(state="normal");
                fp_txt.delete(1.0, "end")
                if devs:
                ...
                fp_txt.configure(state="disabled")
                if res == "PASS" and hasattr(fingerprint_card, 'set_pass'):
                    fingerprint_card.set_pass()
                elif res == "FAIL" and hasattr(fingerprint_card, 'set_fail'):
                    fingerprint_card.set_fail()
    ...
    try:
        if hasattr(fingerprint_card, 'set_pass'):
            _fp_op = fingerprint_card.set_pass;
            _fp_of = fingerprint_card.set_fail
            ...
            fingerprint_card.set_pass = _fp_p;
            fingerprint_card.set_fail = _fp_f
            fingerprint_card.pass_btn.configure(command=_fp_p);
            fingerprint_card.fail_btn.configure(command=_fp_f)
    except:
        pass
    ...
    # TOUCHSCREEN CARD
    # ══════════════════════════════════════════════════════════════════
    touchscreen_card = card(test_stack_col, "👆  Touchscreen", track_key="ts")
    touchscreen_card.pack_forget()
    touchscreen_card.pack(fill="x", padx=0)
    try:
        try:
        ...
        ctk.CTkLabel(ts_header_row, text="👆  Touchscreen", font=ctk.CTkFont(size=14, weight="bold"),
                     text_color="#58a6ff").pack(side="left")
        try:
            # Use lambda so callback resolves after local funcs are defined.
            ctk.CTkButton(ts_header_row, text="⟳", width=28, height=28, fg_color="#444444", hover_color="#555555",
                          command=lambda: _run_touchscreen_test()).pack(side="right")
        except Exception:
            pass
    except Exception:
        pass
    touchscreen_status = ctk.CTkLabel(touchscreen_card, text="Click Run to start touchscreen test.",
                                      font=ctk.CTkFont(size=12), text_color="#9fb3c8")
    ...
    try:
        touchscreen_card.configure(height=136)
        touchscreen_card.pack_propagate(False)
    except Exception:
        pass
    ...
    def _start_touchscreen_test():
        try:
            ...
            if not os.path.exists(ts_path):
                ui_call(lambda: touchscreen_status.configure(text=f"Touchscreentest.py not found: {ts_path}",
                                                             text_color="#ff7b72"))
                return
            ...
            def _open_window():
                try:
                    ...
                    try:
                        ...
                        ui_call(lambda: touchscreen_status.configure(text=f"Error launching test: {e}",
                                                                     text_color="#ff7b72"))
                        try:
                            win.destroy()
                        except Exception:
                            pass
                        return
                    ui_call(lambda: touchscreen_status.configure(
                        text="Touchscreen test running (press Close in test window to finish)", text_color="#7ee787"))
                except Exception as e:
                    ui_call(lambda: touchscreen_status.configure(text=f"Error opening touchscreen window: {e}",
                                                                 text_color="#ff7b72"))

            # open the Toplevel on the UI thread
            app.after(0, _open_window)
        except Exception as e:
            ui_call(lambda: touchscreen_status.configure(text=f"Error preparing touchscreen test: {e}",
                                                         text_color="#ff7b72"))
    ...
    def _run_touchscreen_test():
        try:
            ...
            if _has_touchscreen():
                ui_call(lambda: touchscreen_status.configure(text="Touchscreen detected — starting test...",
                                                             text_color="#7ee787"))
            ...
            def _on_yes():
                try:
                    ...
                    ui_call(
                        lambda: touchscreen_status.configure(text="Operator confirmed touchscreen — starting test...",
                                                             text_color="#7ee787"))
                    threading.Thread(target=_start_touchscreen_test, daemon=True).start()
                except Exception:
                    pass
            ...
            def _on_no():
                try:
                    _create_touchscreen_buttons()
                    ui_call(
                        lambda: touchscreen_status.configure(text="No touchscreen is detected.", text_color="#9fb3c8"))
                    try:
            ...
            try:
                ...
                pass
            try:
                ctk.CTkButton(ts_btn_row, text="Yes", width=120, height=30, fg_color="#2ecc71", command=_on_yes).pack(
                    side="left", padx=(0, 8))
                ctk.CTkButton(ts_btn_row, text="No", width=120, height=30, fg_color="#ff6b6b", command=_on_no).pack(
                    side="left")
            except Exception:
            ...
            ui_call(
                lambda: touchscreen_status.configure(text="Touchscreen test failed to start.", text_color="#ff7b72"))
    ...
    def _create_touchscreen_buttons():
        try:
            ...
            pass
        try:
            ctk.CTkButton(ts_btn_row, text="Run", width=120, height=30, command=_run_touchscreen_test).pack(side="left",
                                                                                                            padx=(0, 8))
        except Exception:
            pass
    ...
    # PIXEL TEST CARD
    pixel_card = card(test_stack_col, "🧪  Pixel Test", track_key="px")
    pixel_card.pack_forget()
    pixel_card.pack(fill="x", padx=0, pady=(8, 0))
    try:
        try:
        ...
        ctk.CTkLabel(px_header_row, text="🧪  Pixel Test", font=ctk.CTkFont(size=14, weight="bold"),
                     text_color="#58a6ff").pack(side="left")
        try:
            ctk.CTkButton(px_header_row, text="⟳", width=28, height=28, fg_color="#444444", hover_color="#555555",
                          command=lambda: _start_pixel_test()).pack(side="right")
        except Exception:
            pass
    except Exception:
        pass
    try:
        # Reduce pixel card height so it hugs the visible controls.
        pixel_card.configure(height=122)
        pixel_card.pack_propagate(False)
    except Exception:
        pass
    pixel_status = ctk.CTkLabel(pixel_card, text="Start the pixel test to cycle full-screen colors.",
                                font=ctk.CTkFont(size=12), text_color="#9fb3c8")
    ...
    try:
        ...
        header_row.pack(fill="x", padx=14, pady=(10, 6))
        ctk.CTkLabel(header_row, text="📷  Camera Test", font=ctk.CTkFont(size=14, weight="bold"),
                     text_color="#58a6ff").pack(side="left")
        try:
            ctk.CTkButton(header_row, text="⟳", width=28, height=28, fg_color="#444444", hover_color="#555555",
                          command=lambda: threading.Thread(target=_start_camera_preview, daemon=True).start()).pack(
                side="right")
        except Exception:
            pass
    except Exception:
    ...
    ctk.CTkButton(cam_btn_row, text="Start", width=110, height=30,
                  command=lambda: threading.Thread(target=_start_camera_preview, daemon=True).start()).pack(side="left",
                                                                                                            padx=(0, 8))
    ctk.CTkButton(cam_btn_row, text="Capture", width=110, height=30, fg_color="#2f6feb", hover_color="#3b82f6",
                  command=_capture_camera_image).pack(side="left", padx=(0, 8))
    ...
    try:
        ...
        header_row.pack(fill="x", padx=14, pady=(10, 6))
        ctk.CTkLabel(header_row, text="⌨️  Keyboard Test", font=ctk.CTkFont(size=14, weight="bold"),
                     text_color="#58a6ff").pack(side="left")
        ...
        try:
            ctk.CTkButton(header_row, text="⟳", width=28, height=28, fg_color="#444444", hover_color="#555555",
                          command=_kb_refresh_clicked).pack(side="right")
        except Exception:
            pass
    except Exception:
        pass
    ...
    ctk.CTkButton(kb_btn_row, text="Start", width=110, height=30, command=_start_keyboard_module).pack(side="left",
                                                                                                       padx=(0, 8))
    ...
    try:
        ...
        header_row.pack(fill="x", padx=14, pady=(10, 6))
        _register_title_label(ctk.CTkLabel(header_row, text="🔐  Activation", font=ctk.CTkFont(size=14, weight="bold"),
                                           text_color="#58a6ff")).pack(side="left")
        try:
            # Use lambda so callback resolves after local funcs are defined.
            _register_refresh_button(
                ctk.CTkButton(header_row, text="⟳", width=28, height=28, fg_color="#444444", hover_color="#555555",
                              command=lambda: _run_activation_check())).pack(side="right")
        except Exception:
            pass
    except Exception:
        pass
    ...
    act_result_label = _register_subtle_label(
        ctk.CTkLabel(act_card, text="Checking...", font=ctk.CTkFont(size=13), text_color="#9fb3c8", justify="left"))
    ...
    header_row.pack(fill="x", padx=14, pady=(10, 6))
    _register_title_label(ctk.CTkLabel(header_row, text="🛠️  Drivers", font=ctk.CTkFont(size=14, weight="bold"),
                                       text_color="#58a6ff")).pack(side="left")
    ...
    try:
        refresh_btn = _register_refresh_button(
            ctk.CTkButton(header_row, text="⟳", width=28, height=28, fg_color="#444444", hover_color="#555555",
                          command=_drv_refresh_clicked))
        refresh_btn.pack(side="right")
    except Exception:
        pass

    drv_status_label = _register_subtle_label(
        ctk.CTkLabel(drv_card, text="Driver Checker", font=ctk.CTkFont(size=12), text_color="#9fb3c8"))
    ...
    def _append_drv_line(line):
        try:
            lbl = ctk.CTkLabel(drv_output_frame, text=line.strip(), font=ctk.CTkFont(size=10), text_color="#d4af37",
...
"""
    ...
    _register_title_label(ctk.CTkLabel(header_row_gpu, text="🎮  GPU", font=ctk.CTkFont(size=14, weight="bold"),
                                       text_color="#58a6ff")).pack(side="left")
    ...
    try:
        _register_refresh_button(
            ctk.CTkButton(header_row_gpu, text="⟳", width=28, height=28, fg_color="#444444", hover_color="#555555",
                          command=_gpu_refresh_clicked)).pack(side="right")
    except Exception:
        pass

    gpu_status_label = _register_subtle_label(
        ctk.CTkLabel(gpu_card, text="Graphics Controller", font=ctk.CTkFont(size=12), text_color="#9fb3c8"))
    ...
    def _append_gpu_line(line):
        try:
            lbl = ctk.CTkLabel(gpu_output_frame, text=line.strip(), font=ctk.CTkFont(size=10), text_color="#d4af37",
                               justify="left", wraplength=600)
            lbl.pack(anchor="w", pady=2)
    ...
    try:
        ...
        header_row.pack(fill="x", padx=14, pady=(10, 6))
        ctk.CTkLabel(header_row, text="🦠  Virus Scan", font=ctk.CTkFont(size=14, weight="bold"),
                     text_color="#58a6ff").pack(side="left")
        try:
            ctk.CTkButton(header_row, text="⟳", width=28, height=28, fg_color="#444444", hover_color="#555555",
                          command=lambda: threading.Thread(target=_start_windef, daemon=True).start()).pack(
                side="right")
        except Exception:
            pass
    except Exception:
    ...
    def _add_vs_output_line(line):
        try:
            ...
            lbl = ctk.CTkLabel(vs_results_frame, text=line.strip(), font=ctk.CTkFont(size=10), text_color="#d4af37",
                               justify="left", wraplength=600)
            lbl.pack(anchor="w", pady=2)
                ...
                try:
                    while True:
                        ...
                        if "no threats detected" in lower_line:
                            saw_no_threats = True
                        if (
                                "threat" in lower_line and "no threats detected" not in lower_line) or "error" in lower_line:
            ...
            try:
                    ...
                    try:
                        for sub in child.winfo_children():
                            ...
                            if isinstance(txt, str) and txt.strip().startswith(title_prefix):
                                ...
                                has_btn = any(
                                    (hasattr(s, 'cget') and s.cget('text') == '⟳') for s in child.winfo_children())
                                if not has_btn:
                                    try:
                                        ctk.CTkButton(child, text="⟳", width=28, height=28, fg_color="#444444",
                                                      hover_color="#555555", command=cmd).pack(side="right")
                                    except Exception:
                                        pass
                                return
                    except Exception:
                ...
                try:
                    ...
                    header.pack(fill="x", padx=14, pady=(10, 6),
                                before=(card.winfo_children()[0] if card.winfo_children() else None))
                    ctk.CTkLabel(header, text=title_prefix.strip(), font=ctk.CTkFont(size=14, weight="bold"),
                                 text_color="#58a6ff").pack(side="left")
                    try:
                        ctk.CTkButton(header, text="⟳", width=28, height=28, fg_color="#444444", hover_color="#555555",
    ...
    add_sidebar_item("comp", "📋 Form", comp_card)
    add_sidebar_item("net", "📶 Network Adapters", net_card)
    add_sidebar_item("ram", "🧠 Memory", ram_card)
    ...
    sequence_keys = [
        "operator", "ag", "sys", "comp", "net", "ram", "bat", "tp", "spk", "mic", "br",
                ...
                try:
                    if hasattr(ch, 'cget'):
                        ...
                        if any(tok in txt for tok in
                               ('PASS', 'FAIL', 'APROBADO', 'FALLA', '✓', '✗', 'NOT RUN', 'SIN EJECUTAR')):
                            return txt
                except Exception:
                    pass
            return None
    ...
    def _start_card_by_key(key):
        try:
            ...
            if key == 'ram':
                ...
                return True
            if key == 'bat':
                app.after(0, _start_battery_embed)
                return True
            if key == 'smartcard':
                app.after(0, _sc_refresh_logic)
                return True
            if key == 'usb':
                threading.Thread(target=_usbport_refresh, daemon=True).start()
                return True
    ...
    def _do_global_refresh():
        ...
        def _reset_for_restart():
            ...
            # Explicitly clear the Form card's Serial/SKU entries back to
            # the "Waiting for System Info..." state on every refresh.
            try:
                _reset_form_fields()
            except Exception:
                pass
