...
def show_hardware_test_screen():
                ...
                if sys_ok:
                    ...
                    if hasattr(sys_card, 'set_fail'):
                        try:
                            ...
                            pass
                    _set_components_active(False)
                # After System Info finishes, highlight Components only.
                # Battery is intentionally NOT started here — the sequencer
                # moves to bat after comp, preventing any race condition.
                try:
                    def _after_sys_next():
                        try:
    ...
    def _bat_auto():
        # Auto-polling disabled: battery only runs via sequencer or manual ⟳ button.
        pass

    # Battery refresh is NOT started here and NOT auto-polled.
    # It only runs when the sequencer reaches the bat card,
    # or when the user manually presses the ⟳ button.
    ...
    def _run_full_sequence(start_key=None, run_id=None):
        try:
            ...
            while idx < len(keys) and _sequence_running[0] and run_id == _sequence_run_id[0]:
                ...
                # comp is auto-resolved by sys card; skip waiting for a fresh result
                if key == 'comp':
                    _log_sequence("sequence skipping wait for comp (auto-resolved by sys)")
                    time.sleep(0.5)
                    idx += 1
                    continue
