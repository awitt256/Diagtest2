...
def show_hardware_test_screen():
    ...
    ctk.CTkLabel(top_bar, text="Hardware Test Suite — Revision 0.29: SPLIT SYSTEM INFO AND COMPONENTS ADDED REFRESH BUTTON,
...
"""
    ...
    def _do_global_refresh():
        ...
        # Reset ALL sidebar indicators to neutral ○
        ...
        def _run_all():
            try:
                ...
                # System Info (cascades → Components auto-pass)
                try:
                    app.after(200, _start_system_info_once)
                except Exception:
                    pass
                ...
                # Drivers — re-trigger its runner if available
                try:
                    app.after(400, _start_driver_scan)
                except Exception:
                    pass

                # GPU — re-trigger if available
                try:
                    app.after(600, _start_gpu_info)
                except Exception:
                    pass

                # Activation — re-trigger if available
                try:
                    app.after(800, _start_activation_check)
                except Exception:
                    pass

                # Virus Scan — re-trigger if available
                try:
                    app.after(1000, _start_windef)
                except Exception:
                    pass
