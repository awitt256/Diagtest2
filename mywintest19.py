...
import os as _os
_kb2_path = _os.path.join(_os.path.dirname(__file__), "keyboard_gui2.py")
_spec = _importlib_util.spec_from_file_location("keyboard_gui2", _kb2_path)
...
def show_keyboard_tester():
    ...
    try:
        tester = KeyboardGUI2.KeyboardGUI(active_screen)
        ...
        messagebox.showerror("Keyboard Test", f"Could not start embedded Keyboard GUI.\n\n{exc}")
        return_to_main_menu()
        return
...
def show_battery_screen():
    ...
    # Automatic PASS/FAIL indicator (based on health percent)
    battery_result_label = ctk.CTkLabel(active_screen, text="",
                                        font=ctk.CTkFont(size=14, weight="bold"), text_color="#9aa4b2")
    battery_result_label.pack(anchor="w", padx=24, pady=(0, 12))
    ...
    def do_refresh():
        try:
            ...
            def _apply_data():
                ...
                # Set PASS/FAIL based on health_percent (>=60% pass)
                try:
                    hp = data.get("health_percent")
                    level_text = data.get("level_text")
                    # Treat missing level or missing health as FAIL
                    if not hp and (not isinstance(hp, (int, float))):
                        # If level_text shows no percent or explicit unknown, mark fail
                        if (not level_text) or level_text.strip() in ("--%", "Unknown", "N/A"):
                            battery_result_label.configure(text="FAIL  ✖  (Unusable)", text_color="#ff6b6b")
                        else:
                            battery_result_label.configure(text="FAIL  ✖  (Health Unknown)", text_color="#ff6b6b")
                    else:
                        # hp is numeric
                        if hp >= 60:
                            battery_result_label.configure(text=f"PASS  ✔  ({hp:.1f}%)", text_color="#2ecc71")
                        else:
                            battery_result_label.configure(text=f"FAIL  ✖  ({hp:.1f}%)", text_color="#ff6b6b")
                except Exception:
                    battery_result_label.configure(text="Health: Unavailable", text_color="#9aa4b2")
            ui_call(_apply_data)
        except Exception as exc:
...
def show_hardware_test_screen():
    ...
    ctk.CTkLabel(top_bar, text="Hardware Test Suite — Revision 0.16: added Driver Check added pass fail checks  new kb added  ",
    ...
    ctk.CTkButton(ag_btn_row, text="Run", width=120, height=30, command=_run_audiog_clicked).pack(side="left")
    ...
    def _set_components_active(is_active):
        ...
        try:
            if comp_state_label is not None:
                if is_active:
                    comp_state_label.configure(text="Components active (System Info PASS)", text_color="#7ee787")
                else:
    ...
    try:
        ...
        comp_state_label.pack(anchor="w", padx=2, pady=(0, 6))
    ...
    ctk.CTkButton(sys_btn_row, text="Run", width=120, height=30, command=_start_system_info_once).pack(side="left")
    ...
    # allocate a compact fixed height for the embedded touchpad so it fits
    # the card without leaving a large empty area below
    try:
        tp_host.configure(height=420)
        tp_host.pack(fill="both", expand=False, padx=14, pady=(6,0))
        tp_host.pack_propagate(False)
    except Exception:
    ...
    ctk.CTkButton(tp_btn_row, text="Stop", width=120, height=32, fg_color="#444444", hover_color="#555555", command=_stop_touchpad_embed).pack(side="left")
    ...
    # BRIGHTNESS CARD
    # ══════════════════════════════════════════════════════════════════
    brightness_card = card(body, "💡  Brightness")
    ...
    br_canvas = tk.Canvas(br_canvas_frame, height=64, bg="#0d1117", highlightthickness=0)
    ...
    br_refresh_btn = ctk.CTkButton(br_btn_row, text="Refresh", width=100, command=lambda: threading.Thread(target=_refresh_brightness, daemon=True).start())
    br_refresh_btn.pack(side="left", padx=(0,8))
    br_set50_btn = ctk.CTkButton(br_btn_row, text="Set 50%", width=100, command=lambda: threading.Thread(target=_set_brightness, daemon=True).start())
    br_set50_btn.pack(side="left", padx=(0,8))
    ...
    br_stop_btn = ctk.CTkButton(br_btn_row, text="Stop Test", width=120, command=lambda: _stop_brightness_test())
    br_stop_btn.pack(side="left", padx=(0,8))

    # Additional manual pass/fail buttons (besides card status)
    br_pass_manual = ctk.CTkButton(br_btn_row, text="Pass", width=80, command=lambda: (brightness_card.set_pass() if hasattr(brightness_card, 'set_pass') else None))
    br_pass_manual.pack(side="left", padx=(6,6))
    br_fail_manual = ctk.CTkButton(br_btn_row, text="Fail", width=80, fg_color="#ff6b6b", hover_color="#ff5252", command=lambda: (brightness_card.set_fail() if hasattr(brightness_card, 'set_fail') else None))
    br_fail_manual.pack(side="left", padx=(6,6))
    ...
    def _draw_brightness_bar(percent):
        try:
            ...
            w = br_canvas.winfo_width() or 560
            h = br_canvas.winfo_height() or 64
            pad = 12
            bar_h = 18
            bar_y = (h // 2) - (bar_h // 2)
            # Track background
    ...
    # ══════════════════════════════════════════════════════════════════
    # TOUCHSCREEN CARD (embed external Touchscreentest.py when present)
    # ══════════════════════════════════════════════════════════════════
    touchscreen_card = card(body, "👆  Touchscreen")
    ...
    def _start_touchscreen_test():
        try:
            ...
            if not os.path.exists(ts_path):
                ...
                try:
                    if hasattr(touchscreen_card, 'set_fail'):
                        touchscreen_card.set_fail()
                except Exception:
                    pass
                return
    ...
    def _run_touchscreen_test():
        try:
            ...
            if _has_touchscreen():
                ...
                ui_call(lambda: touchscreen_status.configure(text="No touchscreen detected — skipping.", text_color="#9fb3c8"))
                try:
                    if hasattr(touchscreen_card, 'pass_btn') and getattr(touchscreen_card, 'pass_btn') is not None:
                        ui_call(lambda: touchscreen_card.pass_btn.invoke())
                    else:
                        ui_call(lambda: touchscreen_card.set_pass() if hasattr(touchscreen_card, 'set_pass') else None)
                except Exception:
                    pass
        except Exception:
            ui_call(lambda: touchscreen_status.configure(text="Touchscreen detection failed.", text_color="#ff7b72"))

    ctk.CTkButton(ts_btn_row, text="Run", width=120, height=30, command=_run_touchscreen_test).pack(side="left", padx=(0, 8))
    ctk.CTkButton(ts_btn_row, text="Stop", width=120, height=30, fg_color="#444444", hover_color="#555555", command=_stop_touchscreen_test).pack(side="left")

    # ══════════════════════════════════════════════════════════════════
    # PIXEL TEST CARD
    # ══════════════════════════════════════════════════════════════════
    pixel_card = card(body, "🧪  Pixel Test")
    ...
    px_btn_row.pack(anchor="w", padx=14, pady=(0,10))
    ...
    px_stop_btn = ctk.CTkButton(px_btn_row, text="Stop", width=100, state="disabled", command=lambda: _stop_pixel_test())
    px_stop_btn.pack(side="left", padx=(0,8))
    ...
    def _start_pixel_test():
        ...
        try:
            ...
            ui_call(lambda: px_stop_btn.configure(state="normal"))
    ...
    ctk.CTkButton(cam_btn_row, text="Stop", width=110, height=30, fg_color="#444444", hover_color="#555555", command=_stop_camera_preview_local).pack(side="left")
    ...
    ctk.CTkButton(kb_btn_row, text="Stop", width=110, height=30, fg_color="#444444", hover_color="#555555", command=_stop_keyboard_module).pack(side="left")
    ...
    act_card = ctk.CTkFrame(row_frame, fg_color="#161b22", corner_radius=10,
                             border_width=1, border_color="#30363d")
    act_card.pack(side="left", fill="both", expand=True, padx=(0,7))
    ctk.CTkLabel(act_card, text="🔐  Activation", font=ctk.CTkFont(size=14, weight="bold"), text_color="#58a6ff").pack(anchor="w", padx=14, pady=(10,6))
    ...
    act_btn_row = ctk.CTkFrame(act_card, fg_color="transparent")
    act_btn_row.pack(anchor="w", padx=14, pady=(0, 8))
    ...
    ctk.CTkButton(act_btn_row, text="Run", width=120, height=30, command=_run_activation_check).pack(side="left")
    ...
    drv_card = ctk.CTkFrame(row_frame, fg_color="#161b22", corner_radius=10,
                            border_width=1, border_color="#30363d")
    drv_card.pack(side="left", fill="both", expand=True, padx=(7,0))
...
"""
    ...
    # Add a retry/refresh button (mirrors Drivers card behavior)
    try:
        vs_btn_row = ctk.CTkFrame(vs_card, fg_color="transparent")
        vs_btn_row.pack(anchor="w", padx=14, pady=(0, 12))

        def _vs_retry_clicked():
            try:
                # clear previous results
                for lbl in list(vs_output_labels):
                    try:
                        lbl.destroy()
                    except Exception:
                        pass
                vs_output_labels.clear()
                app.after(0, lambda: vs_status.configure(text="Re-running scan...", text_color="#9fb3c8"))
                threading.Thread(target=_start_windef, daemon=True).start()
            except Exception:
                pass

        ctk.CTkButton(vs_btn_row, text="Retry", width=120, height=34, command=_vs_retry_clicked).pack(side="left")
    except Exception:
        pass
