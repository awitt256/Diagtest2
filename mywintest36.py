...
def show_hardware_test_screen():
        ...
        try:
            ...
            def set_pass():
                try:
                    ...
                    try:
                        ...
                        # If this is the Battery card and a sequence is running,
                        # schedule an auto-advance after 3 seconds in case the
                        # sequencer didn't move on its own.
                        try:
                            if track_key == 'bat' and _sequence_running[0] and _current_highlight[0] is f:
                                def _set_auto():
                                    try:
                                        # Mark time when auto-advance was requested
                                        setattr(f, '_auto_advance_requested', time.time())
                                        _log_sequence('battery auto-advance requested')
                                    except Exception:
                                        pass
                                try:
                                    app.after(3000, _set_auto)
                                except Exception:
                                    threading.Thread(target=_set_auto, daemon=True).start()
                        except Exception:
                            pass
                    except Exception:
                        pass
            ...
            def set_fail():
                try:
                    ...
                    try:
                        f._last_status_time = time.time()
                        # Same auto-advance scheduling for FAIL as for PASS
                        try:
                            if track_key == 'bat' and _sequence_running[0] and _current_highlight[0] is f:
                                def _set_auto_fail():
                                    try:
                                        setattr(f, '_auto_advance_requested', time.time())
                                        _log_sequence('battery auto-advance requested (fail)')
                                    except Exception:
                                        pass
                                try:
                                    app.after(3000, _set_auto_fail)
                                except Exception:
                                    threading.Thread(target=_set_auto_fail, daemon=True).start()
                        except Exception:
                            pass
                    except Exception:
                        pass
    ...
    def _spk_on_finished():
        ...
        # If looping is enabled and the card hasn't been marked PASS/FAIL, restart playback
        try:
            if spk_loop[0]:
    ...
    def _spk_play():
        ...
        # enable looping when play is started via sequence or user
        spk_loop[0] = True
            ...
            try:
                ...
                if _extract_card_result(txt):
                    ...
                    try:
                        ...
                        last = 0
                    # Normal immediate acceptance when result was set after we started
                    if last >= min_time:
                        _log_sequence(f"wait result for card: {txt}")
                        return txt
                    # Special-case: if an auto-advance was requested for this card
                    # (used primarily for Battery), allow returning once the
                    # auto-advance timestamp is set and it occurred after min_time.
                    try:
                        auto_ts = getattr(card, '_auto_advance_requested', 0) or 0
                    except Exception:
                        auto_ts = 0
                    if auto_ts and auto_ts >= min_time:
                        _log_sequence(f"wait auto-advance for card: {txt} auto_ts={auto_ts}")
                        return txt
                    else:
